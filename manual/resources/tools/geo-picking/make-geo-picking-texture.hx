...
class Main {
	static function main() {
		var size:Int = 4096;
		var pickCtx:CanvasRenderingContext2D = cast document.querySelector( '#pick' ).getContext( '2d' );
		pickCtx.canvas.width = size;
		pickCtx.canvas.height = size;

		var outlineCtx:CanvasRenderingContext2D = cast document.querySelector( '#outline' ).getContext( '2d' );
		...
		var workCtx:CanvasRenderingContext2D = cast document.createElement( 'canvas' ).getContext( '2d' );
		...
		var geoHandlers:Map<String, Dynamic> = [
		...
		function draw(area:Dynamic) {
			...
			var countryInfo:Dynamic = countriesById[countryData.get(name).id - 1];

			var handler:Dynamic = geoHandlers.get(type);
			if (handler == null) {
			...
			countryInfo.population = { '2005': properties.POP2005 };
			...
			var color:Array<Int> = countryData.get(name).color;
		...
		var source:Promise<Void> = shapefile.open('TM_WORLD_BORDERS-0.3.shp');
		var areas:Array<Dynamic> = [];
		source.then((source) -> {
			var i:Int = 0;
			while (true) {
				var {done, value} = source.read();
				if (done) break;
				areas.push(value);
				draw(value);
				if (i % 20 == 19) wait();
			...
			function getId(imageData:ImageData, x:Int, y:Int):Int {
				var off:Int = ((y + imageData.height) % imageData.height) * imageData.width + ((x + imageData.width) % imageData.width) * 4;
				return imageData.data[off + 0] + imageData.data[off + 1] * 256 + imageData.data[off + 2] * 256 * 256 + imageData.data[off + 3] * 256 * 256 * 256;
			}
			...
			for (y in 0...outline.height) {
				for (x in 0...outline.width) {
					...
					var v:Int = s;
					if ((s && r && d) || (s && l && d) || (s && r && u) || (s && l && u)) v = 0;
					putPixel(outline, x, y, [v, v, v, v]);
				}
			}
