...
class SidebarProjectImage {
    public function new(editor:Dynamic) {
        ...
        renderButton.onClick(function() {
            ...
            var loader = new three.js.loaders.ObjectLoader();
            ...
            var renderer = new three.js.WebGLRenderer({ antialias: true });
            ...
            var width = imageWidth.getValue() / Lib.window.devicePixelRatio;
            var height = imageHeight.getValue() / Lib.window.devicePixelRatio;

            var left = (Lib.window.screen.width - width) / 2;
            var top = (Lib.window.screen.height - height) / 2;

            var output = Lib.window.open('', '_blank', 'location=no,left=$left,top=$top,width=$width,height=$height');

            var meta = Lib.document.createElement('meta');
            meta.name = 'viewport';
            ...
            switch (Std.parseInt(shadingTypeSelect.getValue())) {
                ...
                /*case 1: // REALISTIC
                    ...
                    var status = Lib.document.createElement('div');
                    ...
                    function animate() {
                        ...
                        Lib.requestAnimationFrame(animate);
                        pathtracer.update();
                    }
