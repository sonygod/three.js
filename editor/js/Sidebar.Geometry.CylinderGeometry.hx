...
```
Note that I've assumed that the Haxe compiler is configured to target JavaScript, and that the `three.js` and `js.lib.ui` libraries are available. Additionally, I've used the `Math.POSITIVE_INFINITY` constant to represent infinity, as there is no direct equivalent in Haxe.