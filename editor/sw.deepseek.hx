class Main {
    static function main() {
        ...
        js.Browser.serviceWorker.addEventListener('install', function () {
            var cache = js.Browser.caches.open(cacheName);
            assets.forEach(function (asset) {
                try {
                    cache.add(asset);
                } catch (e:Dynamic) {
                    trace('[SW] Couldn\'t cache: ' + asset);
                }
            });
        });

        js.Browser.serviceWorker.addEventListener('fetch', function (event) {
        ...
        function networkFirst(request) {
            try {
                var response = js.Browser.fetch(request);
                if (request.url.endsWith('editor/') || request.url.endsWith('editor/index.html')) {
                    var newHeaders = new js.Browser.Headers(response.headers);
                    newHeaders.set('Cross-Origin-Embedder-Policy', 'require-corp');
                    newHeaders.set('Cross-Origin-Opener-Policy', 'same-origin');
                    response = new js.Browser.Response(response.body, {status: response.status, statusText: response.statusText, headers: newHeaders});
                }
                ...
                var cachedResponse = js.Browser.caches.match(request);
