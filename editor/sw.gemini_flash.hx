...
class ServiceWorker {
	...
	static function main() {
		Window.window.addEventListener("install", install, false);
		Window.window.addEventListener("fetch", fetch, false);
	}

	static function install(event:HXObject) {
		var cache:HXObject = Browser.window.caches.open(cacheName);
		assets.forEach(function (asset:String) {
			cache.add(asset);
		});
	}

	static function fetch(event:HXObject) {
		var request:HXObject = event.request;
		if (request.url.startsWith("chrome-extension")) return;
		event.respondWith(networkFirst(request));
	}

	static function networkFirst(request:HXObject) {
		var response:HXObject;
		try {
			response = Browser.window.fetch(request);
			if (request.url.endsWith("editor/") || request.url.endsWith("editor/index.html")) {
				var newHeaders:HXObject = new Headers(response.headers);
			...
			if (request.method == "GET") {
				Browser.window.caches.open(cacheName).put(request, response.clone());
			}
			return response;
		} catch (e:Dynamic) {
			var cachedResponse:HXObject = Browser.window.caches.match(request);
