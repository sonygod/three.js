...
class SMAAEdgesShader {
    ...
    public var uniforms: { [key: String]: { value: dynamic /*Texture | Vector2*/ }; } = {
        "tDiffuse": { value: null },
...
class SMAAWeightsShader {
    ...
    public var uniforms: { [key: String]: { value: dynamic /*Texture*/ }; } = {
        "tDiffuse": { value: null },
        "tArea": { value: null },
        "tSearch": { value: null },
        "resolution": { value: new Vector2(1 / 1024, 1 / 512) }
    ...
    ";
    public var fragmentShader: String = "#define SMAASampleLevelZeroOffset(tex, coord, offset) texture2D(tex, coord + float(offset) * resolution, 0.0)\n" +
        "uniform sampler2D tDiffuse;\n" +
        ...
        "        * sample between edge, thus fetching four edges in a row. Sampling with\n" +
        "        * different offsets in each direction allows to disambiguate which edges\n" +
        "        * are active from the four fetched ones.\n" +
        "        */\n" +
        "    vec2 e = vec2(0.0, 1.0);\n" +
        "\n" +
