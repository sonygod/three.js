import three.js.examples.jsm.shaders.GTAOShader;
import three.js.examples.jsm.shaders.GTAODepthShader;
import three.js.examples.jsm.shaders.GTAOBlendShader;
import three.js.examples.jsm.shaders.generateMagicSquareNoise;
import three.js.examples.jsm.shaders.generateMagicSquare;
import three.js.examples.jsm.shaders.DataTexture;
import three.js.examples.jsm.shaders.Matrix4;
import three.js.examples.jsm.shaders.RepeatWrapping;
import three.js.examples.jsm.shaders.Vector2;
import three.js.examples.jsm.shaders.Vector3;
...
class GTAOShader {
	...
	public static var name:String = 'GTAOShader';
...
class GTAODepthShader {
	...
	public static var name:String = 'GTAODepthShader';
...
class GTAOBlendShader {
	...
	public static var name:String = 'GTAOBlendShader';
...
function generateMagicSquareNoise( size:Float = 5 ) {

	const noiseSize:Int = Math.floor( size ) % 2 === 0 ? Math.floor( size ) + 1 : Math.floor( size );
	const magicSquare:Array<Int> = generateMagicSquare( noiseSize );
	const noiseSquareSize:Int = magicSquare.length;
	const data:Uint8Array = new Uint8Array( noiseSquareSize * 4 );

	for ( let inx:Int = 0; inx < noiseSquareSize; ++ inx ) {

		const iAng:Int = magicSquare[ inx ];
		const angle:Float = ( 2 * Math.PI * iAng ) / noiseSquareSize;
		const randomVec:Vector3 = new Vector3(
			Math.cos( angle ),
			Math.sin( angle ),
			0
		).normalize();
	...
	const noiseTexture:DataTexture = new DataTexture( data, noiseSize, noiseSize );
...
function generateMagicSquare( size:Int ) {

	const noiseSize:Int = Math.floor( size ) % 2 === 0 ? Math.floor( size ) + 1 : Math.floor( size );
	const noiseSquareSize:Int = noiseSize * noiseSize;
	const magicSquare:Array<Int> = Array( noiseSquareSize ).fill( 0 );
	let i:Int = Math.floor( noiseSize / 2 );
	let j:Int = noiseSize - 1;

	for ( let num:Int = 1; num <= noiseSquareSize; ) {
