...
class ImprovedNoise {
    ...
    public function noise( x:Float, y:Float, z:Float ) {
        var floorX = Std.int(x), floorY = Std.int(y), floorZ = Std.int(z);
        ...
        var A = _p[ X ] + Y, AA = _p[ A ] + Z, AB = _p[ A + 1 ] + Z, B = _p[ X + 1 ] + Y, BA = _p[ B ] + Z, BB = _p[ B + 1 ] + Z;
    ...
    private function grad( hash:Int, x:Float, y:Float, z:Float ) {
        var h = hash & 15;
