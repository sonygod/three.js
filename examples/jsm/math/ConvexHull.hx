...
class ConvexHull {
	...
	private var _line3:Line3 = new Line3();
	...
	public function setFromPoints( points:Array<Vector3> ):ConvexHull {
		if ( points.length >= 4 ) {
			this.makeEmpty();
			for ( p in points ) {
				this.vertices.push( new VertexNode(p) );
			}
			this.compute();
		}
		return this;
	...
	public function setFromObject( object ):ConvexHull {
	...
	public function containsPoint( point:Vector3 ):Bool {
		for ( f in faces ) {
			if ( f.distanceToPoint(point) > this.tolerance ) return false;
		}
		return true;
	}
	...
	public function intersectRay( ray:Line3, target:Vector3 ):Vector3 {
		...
		for ( f in faces ) {
			...
			var vN:Float = f.distanceToPoint(ray.origin);
			var vD:Float = f.normal.dot(ray.direction);
	...
	public function deleteFaceVertices( face:Face, absorbingFace:Face ):ConvexHull {
		...
		if ( faceVertices !== undefined ) {
			if ( absorbingFace === undefined ) {
				...
				while ( vertex !== null ) {
					// buffer the subsequent vertex at this point because the 'vertex.next' reference
	...
	public function computeExtremes():Array<Array<VertexNode>> {
		var min:Array<VertexNode> = [];
		var max:Array<VertexNode> = [];
		// initially assume that the first vertex is the min/max
		for ( i in 0...3 ) {
			min[i] = max[i] = this.vertices[0];
		}
		min[0].copy(this.vertices[0].point);
		max[0].copy(this.vertices[0].point);
		...
		for ( i in 0...this.vertices.length ) {
			...
			for ( j in 0...3 ) {
				if ( point.getComponent(j) < min[j].point.getComponent(j) ) {
					min[j].point.setComponent(j, point.getComponent(j));
					min[j] = vertex;
				}
			}
			// update the max coordinates
			for ( j in 0...3 ) {
				if ( point.getComponent(j) > max[j].point.getComponent(j) ) {
					max[j].point.setComponent(j, point.getComponent(j));
					max[j] = vertex;
				}
			}
		}
		// use min/max vectors to compute an optimal epsilon
		this.tolerance = 3*Number.EPSILON*(Math.max(Math.abs(min[0].point.x), Math.abs(max[0].point.x)) +
			Math.max(Math.abs(min[0].point.y), Math.abs(max[0].point.y)) +
			Math.max(Math.abs(min[0].point.z), Math.abs(max[0].point.z)));
		return {min:min, max:max};
	}

	public function computeInitialHull():ConvexHull {
		var extremes:Array<Array<VertexNode>> = this.computeExtremes();
		...
		var v2:VertexNode = null;
		var v3:VertexNode = null;
	...
	public function nextVertexToAdd():VertexNode {
		...
		if ( this.assigned.isEmpty() === false ) {
			var eyeVertex:VertexNode, maxDistance:Float = 0;
	...
	public function addAdjoiningFace( eyeVertex:VertexNode, horizonEdge:HalfEdge ):HalfEdge {
		...
		face.getEdge(2).setTwin(horizonEdge.twin);
...
class HalfEdge {
	...
	public function lengthSquared():Float {
		...
		var tail:VertexNode = this.tail();
		if (tail !== null) {
			return head.point.distanceToSquared(tail.point);
		}
		return -1;
	}
