...
class ImprovedNoise {
    ...
    static function grad( hash:Int, x:Float, y:Float, z:Float ):Float {
        var h:Int = hash & 15;
        var u:Float = h < 8 ? x : y, v:Float = h < 4 ? y : h == 12 || h == 14 ? x : z;
        return ( ( h & 1 ) == 0 ? u : - u ) + ( ( h & 2 ) == 0 ? v : - v );
    }

    public function noise( x:Float, y:Float, z:Float ):Float {
        var floorX:Int = Math.floor( x ), floorY:Int = Math.floor( y ), floorZ:Int = Math.floor( z );

        var X:Int = floorX & 255, Y:Int = floorY & 255, Z:Int = floorZ & 255;

        x -= floorX;
        y -= floorY;
        z -= floorZ;

        var xMinus1:Float = x - 1, yMinus1:Float = y - 1, zMinus1:Float = z - 1;

        var u:Float = fade( x ), v:Float = fade( y ), w:Float = fade( z );

        var A:Int = _p[ X ] + Y, AA:Int = _p[ A ] + Z, AB:Int = _p[ A + 1 ] + Z, B:Int = _p[ X + 1 ] + Y, BA:Int = _p[ B ] + Z, BB:Int = _p[ B + 1 ] + Z;
