...
import haxe.ui.threejs.core.BufferAttribute;
import haxe.ui.threejs.core.BufferGeometry;
import haxe.ui.threejs.math.Color;
import haxe.ui.threejs.math.Quaternion;
import haxe.ui.threejs.core.Raycaster;
import haxe.ui.threejs.math.Vector3;
...
class RollerCoasterGeometry extends BufferGeometry {
    ...
    public function new(curve:Dynamic, divisions:Int) {
        ...
        var color1:Array<Float> = [1.0, 1.0, 1.0];
        var color2:Array<Float> = [1.0, 1.0, 0.0];
        ...
        function drawShape(shape:Array<Vector3>, color:Array<Float>) {
            ...
            for (var j:Int = shape.length - 1; j >= 0; j--) {
                vector.copy(shape[j]);
                vector.applyQuaternion(quaternion);
                vector.add(point);
        ...
        function extrudeShape(shape:Array<Vector3>, offset:Vector3, color:Array<Float>) {
            for (var j:Int = 0, jl:Int = shape.length; j < jl; j++) {
                var point1:Vector3 = shape[j];
                var point2:Vector3 = shape[(j + 1) % jl];
...
// Other classes like RollerCoasterLiftersGeometry, RollerCoasterShadowGeometry, SkyGeometry, TreesGeometry can be similarly converted.