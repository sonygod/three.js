package three.js.examples.jsw.misc;
...
class MorphBlendMesh extends Mesh {
    ...
    public function new(geometry:Geometry, material:Material) {
        ...
        var numFrames:Int = Lambda.count(Object.keys(morphTargetDictionary));
        ...
        for (key in morphTargetDictionary.keys()) {
            ...
            if (chunks.length > 1) {
                ...
                if (!frameRanges.exists(name)) {
                    frameRanges.set(name, {start: Math.POSITIVE_INFINITY, end: Math.NEGATIVE_INFINITY});
                }

                var range:Dynamic = frameRanges.get(name);
    ...
    public function setAnimationDirectionForward(name:String) {
        var animation:Dynamic = animationsMap.get(name);
