  ...
  public static function createButton(renderer:Dynamic, sessionInit:Dynamic = {}):Element {
    ...
    function showStartAR(/*device*/) {
      if (sessionInit.domOverlay == null) {
        var overlay:Element = Browser.document.createElement("div");
        overlay.style.display = "none";
        Browser.document.body.appendChild(overlay);

        var svg:SVGSVGElement = Browser.document.createElementNS("http://www.w3.org/2000/svg", "svg");
        svg.setAttribute("width", "38");
        svg.setAttribute("height", "38");
        svg.style.position = "absolute";
        svg.style.right = "20px";
        svg.style.top = "20px";
        svg.addEventListener("click", function(event:Event) {
          currentSession.end();
        });
        overlay.appendChild(svg);

        var path:SVGPathElement = Browser.document.createElementNS("http://www.w3.org/2000/svg", "path");
