...
class STLExporter {
    ...
    public function parse(scene:Dynamic, options:Dynamic = {}):Dynamic {
        ...
        if (binary) {
            ...
            var arrayBuffer:js.lib.ArrayBuffer = new js.lib.ArrayBuffer(bufferLength);
            output = new js.lib.DataView(arrayBuffer);
        ...
        for (i in 0...objects.length) {
            ...
            if (index != null) {
                // indexed geometry
                for (j in 0...(index.count / 3)) {
                    var a:Int = index.getX(j * 3);
                    var b:Int = index.getX(j * 3 + 1);
                    var c:Int = index.getX(j * 3 + 2);

                    writeFace(a, b, c, positionAttribute, object);
                }
            } else {
                // non-indexed geometry
                for (j in 0...(positionAttribute.count / 3)) {
                    var a:Int = j * 3;
                    var b:Int = j * 3 + 1;
                    var c:Int = j * 3 + 2;
