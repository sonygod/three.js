...
class USDZExporter {
	...
	public function parse(scene:Dynamic, onDone:Dynamic, onError:Dynamic, options:Dynamic = {}) {
		this.parseAsync(scene, options).then(onDone).catch(onError);
	}

	public async function parseAsync(scene:Dynamic, options:Dynamic = {}) {
		...
		for (var id in textures) {
			...
			var blob = await haxe.Http.request(canvas, 'blob', { format: 'image/png' });

			files[`textures/Texture_${id}.png`] = new Uint8Array(Blob.fromData(blob));
		}
