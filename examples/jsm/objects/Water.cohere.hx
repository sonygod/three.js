...
import h3d.Vector3;
import h3d.Vector4;
import h3d.shader.Material;
import h3d.shader.Uniform;
import h3d.shader.UniformType;
import h3d.shader.Uniforms;
import h3d.shader.WebGLRenderTarget;

class Water extends h3d.Mesh {
    public var isWater:Bool;
    private var scope:Water;
    private var textureWidth:Int;
    private var textureHeight:Int;
    private var clipBias:Float;
    private var alpha:Float;
    private var time:Float;
    private var normalSampler:h3d.Texture;
    private var sunDirection:Vector3;
    private var sunColor:h3d.Color;
    private var waterColor:h3d.Color;
    private var eye:Vector3;
    private var distortionScale:Float;
    private var side:h3d.MaterialSide;
    private var fog:Bool;
    ...
    private var mirrorCamera:h3d.Camera;
    private var renderTarget:WebGLRenderTarget;
    private var mirrorShader:h3d.Shader;
    private var material:Material;

    public function new(geometry:h3d.Geometry, options:Dynamic) {
        super(geometry);
        isWater = true;
        scope = this;
        ...
        sunColor = new h3d.Color(options.sunColor ?? 0xffffff);
        waterColor = new h3d.Color(options.waterColor ?? 0x7F7F7F);
        eye = options.eye ?? new Vector3(0, 0, 0);
        distortionScale = options.distortionScale ?? 20.0;
        side = options.side ?? h3d.FrontSide;
        ...
        mirrorCamera = new h3d.PerspectiveCamera();
        ...
        mirrorShader = {
            ...
            uniforms: mergeUniforms([
                ...
                Uniforms.lights,
                {
                    'normalSampler': { type: UniformType.Texture, value: null },
                    'mirrorSampler': { type: UniformType.Texture, value: null },
                    'alpha': { type: UniformType.Float, value: 1.0 },
                    'time': { type: UniformType.Float, value: 0.0 },
                    'size': { type: UniformType.Float, value: 1.0 },
                    'distortionScale': { type: UniformType.Float, value: 20.0 },
                    'textureMatrix': { type: UniformType.Matrix4, value: new Matrix4() },
                    'sunColor': { type: UniformType.Color, value: new h3d.Color(0x7F7F7F) },
                    'sunDirection': { type: UniformType.Vector3, value: new Vector3(0.70707, 0.70707, 0) },
                    'eye': { type: UniformType.Vector3, value: new Vector3() },
                    'waterColor': { type: UniformType.Color, value: new h3d.Color(0x555555) }
                }
            ]),
            vertexShader: /* glsl */`
        ...
        material = new Material( {
            ...
            uniforms: cloneUniforms(mirrorShader.uniforms),
    ...
    private function mergeUniforms(uniforms:Array<Uniform>):Uniforms {
        var mergedUniforms = new Uniforms();
        for (uniform in uniforms) {
            mergedUniforms.push(uniform);
        }
        return mergedUniforms;
    }

    private function cloneUniforms(uniforms:Uniforms):Uniforms {
        var clonedUniforms = new Uniforms();
        for (uniform in uniforms) {
            clonedUniforms.push({
                name: uniform.name,
                type: uniform.type,
