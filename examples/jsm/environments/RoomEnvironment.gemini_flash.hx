...
class RoomEnvironment extends Scene {
  public function new(renderer:Dynamic = null) {
    super();
    ...
    if (renderer != null && Reflect.field(renderer, '_useLegacyLights') == false) intensity = 900;

    var mainLight = new PointLight(0xffffff, intensity, 28, 2);
  ...
  public function dispose() {
    var resources = new haxe.ds.Set();

    this.traverse(function(object:Object3D) {
      if (Reflect.is(object, Mesh)) {
        resources.add(Reflect.field(object, 'geometry'));
        resources.add(Reflect.field(object, 'material'));
      }
    });
...
function createAreaLightMaterial(intensity:Float):MeshBasicMaterial {
  ...
  material.color = new Color(intensity);
  return material;
}
...
export {RoomEnvironment, BackSide};