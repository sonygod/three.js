...
class LightProbeGenerator {
    ...
    static function fromCubeTexture(cubeTexture:Dynamic):LightProbe {
        ...
        for (var faceIndex:Int = 0; faceIndex < 6; faceIndex++) {
            ...
            for (var i:Int = 0, il:Int = data.length; i < il; i += 4) { // RGBA assumed
                // pixel color
                ...
                var row:Float = 1 - (Std.int(pixelIndex / imageWidth) + 0.5) * pixelSize;
    ...
    static function fromCubeRenderTarget(renderer:Dynamic, cubeRenderTarget:Dynamic):LightProbe {
        ...
        var dataType:Class<Dynamic> = Type.getClass(cubeRenderTarget.texture.type);
        ...
        for (var faceIndex:Int = 0; faceIndex < 6; faceIndex++) {
            ...
            if (dataType == HalfFloatType) {
                data = new Array<Int>(imageWidth * imageWidth * 4);
            } else {
...
function convertColorToLinear(color:Color, colorSpace:Class<Dynamic>):Color {
    switch (colorSpace) {
        case SRGBColorSpace:
...
class LightProbeGeneratorExports {
    public static var LightProbeGenerator:Class<LightProbeGenerator> = LightProbeGenerator;
}