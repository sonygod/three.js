...
class WebGPUBackend extends Backend {
	...
	public async function init(renderer:Renderer) {
		...
		const context = (parameters.context !== null) ? parameters.context : renderer.domElement.getContext('webgpu');
	...
	public async function getArrayBufferAsync(attribute:Attribute) {
	...
	private function _getDefaultRenderPassDescriptor() {
	...
	private function _getRenderPassDescriptor(renderContext:RenderContext) {
	...
	public function beginRender(renderContext:RenderContext) {
	...
	public function finishRender(renderContext:RenderContext) {
	...
	public function isOccluded(renderContext:RenderContext, object:Object3D) {
	...
	public async function resolveOccludedAsync(renderContext:RenderContext) {
		...
		if (currentOcclusionQueryBuffer && currentOcclusionQueryObjects) {
			...
			const occluded = new WeakSet<Dynamic>();
			...
			for (i in 0...currentOcclusionQueryObjects.length) {
	...
	public function updateViewport(renderContext:RenderContext) {
	...
	public function clear(color:Bool, depth:Bool, stencil:Bool, renderTargetData:Dynamic = null) {
		...
		if (renderTargetData === null) {
			...
			if (color) {
				...
				for (texture in renderTargetData.textures) {
	...
	public function beginCompute(computeGroup:ComputeGroup) {
	...
	public function compute(computeGroup:ComputeGroup, computeNode:ComputeNode, bindings:Dynamic, pipeline:Dynamic) {
	...
	public function finishCompute(computeGroup:ComputeGroup) {
	...
	public function draw(renderObject:RenderObject, info:Info) {
		...
		if (contextData.occlusionQuerySet !== null) {
	...
	public function needsRenderUpdate(renderObject:RenderObject):Bool {
	...
	public function getRenderCacheKey(renderObject:RenderObject):String {
	...
	public function copyTextureToBuffer(texture:Texture, x:Int, y:Int, width:Int, height:Int):Dynamic {

		return this.textureUtils.copyTextureToBuffer(texture, x, y, width, height);

	}

	private function initTimestampQuery(renderContext:RenderContext, descriptor:Dynamic) {
	...
	private function prepareTimestampBuffer(renderContext:RenderContext, encoder:Dynamic) {
	...
	public async function resolveTimestampAsync(renderContext:RenderContext, type:String = 'render') {
		if (!this.hasFeature(GPUFeatureName.TimestampQuery) || !this.trackTimestamp) return;

		const renderContextData = this.get(renderContext);
		const { currentTimestampQueryBuffer } = renderContextData;

		if (currentTimestampQueryBuffer === null) return;
		...
		try {
			...
			trace('Error mapping buffer: ${error}');
	...
	public function createNodeBuilder(object:Object3D, renderer:Renderer, scene:Scene = null):WGSLNodeBuilder {
	...
	public function createRenderPipeline(renderObject:RenderObject, promises:Dynamic) {

		this.pipelineUtils.createRenderPipeline(renderObject, promises);

	}

	public function createComputePipeline(computePipeline:ComputePipeline, bindings:Dynamic) {
	...
	public function createIndexAttribute(attribute:Attribute) {
	...
	public function createStorageAttribute(attribute:Attribute) {
	...
	public function copyTextureToTexture(srcTexture:Texture, dstTexture:Texture, srcRegion:Dynamic = null, dstPosition:Dynamic = null, level:Int = 0) {
	...
	public function copyFramebufferToTexture(texture:Texture, renderContext:RenderContext) {
		...
		if (sourceGPU.format !== destinationGPU.format) {
			...
			trace('WebGPUBackend: copyFramebufferToTexture: Source and destination formats do not match.', sourceGPU.format, destinationGPU.format);
