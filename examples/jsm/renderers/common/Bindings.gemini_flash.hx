...
import Constants from "./Constants.hx";

class Bindings extends DataMap {

	public backend:Dynamic;
	public textures:Dynamic;
	public pipelines:Dynamic;
	public attributes:Dynamic;
	...
	private function _init(bindings:Array<Dynamic>):Void {
		for (binding in bindings) {
			if (binding.isSampledTexture) {
				...
				this.attributes.update(attribute, Constants.AttributeType.STORAGE);
			}
		}
	}
	...
	private function _update(object:Dynamic, bindings:Array<Dynamic>):Void {
		...
		for (binding in bindings) {
			...
			if (binding.isUniformBuffer) {
				...
				if (backend.isWebGPUBackend == true && textureData.texture == null && textureData.externalTexture == null) {
