package three.js.examples.jm.curves;
...
class NURBSVolume {
    ...
    public function new(degree1:Int, degree2:Int, degree3:Int, knots1:Array<Float>, knots2:Array<Float>, knots3:Array<Float>, controlPoints:Array<Array<Array<Vector4>>>) {
        ...
        var len3:Int = knots3.length - degree3 - 1;
        ...
        for (i in 0...len1) {
            ...
            for (j in 0...len2) {
                ...
                for (k in 0...len3) {
                    var point:Vector4 = controlPoints[i][j][k];
    ...
    public function getPoint(t1:Float, t2:Float, t3:Float, target:Vector4) {
        var u:Float = knots1[0] + t1 * (knots1[knots1.length - 1] - knots1[0]); // linear mapping t1->u
        var v:Float = knots2[0] + t2 * (knots2[knots2.length - 1] - knots2[0]); // linear mapping t2->v
        var w:Float = knots3[0] + t3 * (knots3[knots3.length - 1] - knots3[0]); // linear mapping t3->w
