...
class MeshPostProcessingMaterial extends MeshPhysicalMaterial {
    ...
    var _aoPassMap:Texture;
    var aoPassMapScale:Float;
    var _shader:Shader;

    public function new(parameters:Dynamic) {
        _aoPassMap = parameters.aoPassMap;
        aoPassMapScale = parameters.aoPassMapScale != null ? parameters.aoPassMapScale : 1.0;
        Reflect.deleteField(parameters, "aoPassMap");
        Reflect.deleteField(parameters, "aoPassMapScale");
        super(parameters);
        onBeforeCompile = _onBeforeCompile;
        customProgramCacheKey = _customProgramCacheKey;
    }
    ...
    public function set_aoPassMap(value:Texture):Void {
        _aoPassMap = value;
        needsUpdate = true;
        _setUniforms();
    }

    function _customProgramCacheKey():String {
        return _aoPassMap != null ? 'aoPassMap' : '';
    }

    function _onBeforeCompile(shader:Shader):Void {
        _shader = shader;
        if (_aoPassMap != null) {
    ...
    function _setUniforms():Void {
        if (_shader != null) {
            _shader.uniforms.set('tAoPassMap', _aoPassMap);
            _shader.uniforms.set('aoPassMapScale', aoPassMapScale);
        }
    }

    static var aomap_pars_fragment_replacement:String = "
#ifdef USE_AOMAP
...
";
    ...
    static var aomap_fragment_replacement:String = "
