...
class TextureNode extends UniformNode {
	...
	public var referenceNode:TextureNode;
	...
	function set_value(value:Dynamic):Void {
		if (this.referenceNode != null) {
	...
	public function getNodeType(builder:Dynamic):String {
		if (cast this.value.isDepthTexture : Bool) return "float";
		return "vec4";
	}
	...
	public function getTransformedUV(uvNode:UVNode):ShaderNode {
		var texture = this.value;
		return ShaderNode.uniform(texture.matrix).mul(ShaderNode.vec3(uvNode, 1)).xy;
	}
	...
	public function generate(builder:Dynamic, output:String):String {
		...
		var texture = this.value;
		if (texture == null || !cast texture.isTexture : Bool) {
		...
		if (output == "sampler") {
			...
			if (propertyName == null) {
				var {uvNode, levelNode, compareNode, depthNode, gradNode} = properties;
	...
	public function uv(uvNode:UVNode):ShaderNode {
	...
	public function blur(levelNode:Node):ShaderNode {
		var textureNode = this.clone();
	...
	public function size(levelNode:Node):ShaderNode {
		return TextureSizeNode.textureSize(this, levelNode);
	}

	public function compare(compareNode:Node):ShaderNode {
		var textureNode = this.clone();
	...
	public function grad(gradNodeX:Node, gradNodeY:Node):ShaderNode {
		var textureNode = this.clone();
	...
	public function depth(depthNode:Node):ShaderNode {
		var textureNode = this.clone();
...
export var TextureNode = TextureNode;
export var texture = ShaderNode.nodeProxy(TextureNode);
export function textureLoad(...params:Dynamic):ShaderNode {
	return texture(...params).setSampler(false);
}

//export function textureLevel(value:Dynamic, uv:Dynamic, level:Dynamic):ShaderNode {
//	return texture(value, uv).level(level);
//}

export function sampler(aTexture:Dynamic):ShaderNode {
	return (cast aTexture.isNode : Bool ? aTexture : texture(aTexture)).convert("sampler");
}

ShaderNode.addNodeElement("texture", texture);
//ShaderNode.addNodeElement("textureLevel", textureLevel);
