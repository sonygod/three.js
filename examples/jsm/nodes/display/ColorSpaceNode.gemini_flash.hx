...
import { addNodeElement, tslFn, nodeObject, nodeProxy, vec4 } from "../shadernode/ShaderNode";
...
class ColorSpaceNode extends TempNode {
  ...
  public constructor(method: String, node: TempNode) {
    super("vec4");
...
export const linearToColorSpace = (node: TempNode, colorSpace: Dynamic) =>
  nodeObject(new ColorSpaceNode(getMethod(LinearSRGBColorSpace, colorSpace), nodeObject(node)));
export const colorSpaceToLinear = (node: TempNode, colorSpace: Dynamic) =>
