...
class AnamorphicNode extends TempNode {
  ...
  private _invSize:TempNode;
  private _textureNode:TempNode;
  private _material:Dynamic<any>;
  ...
  public function setup(builder:Dynamic):TempNode {
    ...
    if (!Std.isOfType(textureNode, TempNode)) {
    ...
    var anamorph = tslFn(function():TempNode {
      var samples = this.samples;
      var halfSamples = Math.floor(samples / 2);

      var total = vec3(0).toVar();

      loop({start: -halfSamples, end: halfSamples}, function({i}:Dynamic):Void {
