...
import { addNodeClass, addNodeElement, nodeObject, float, mat3, vec3, tslFn } from '../shadernode/ShaderNode.hx';
import { rendererReference } from '../accessors/RendererReferenceNode.hx';
import { clamp, log2, max, pow, mul } from '../math/MathNode.hx';
...
const toneMappingLib = {
	[$type<LinearToneMapping>()]: LinearToneMappingNode,
	[$type<ReinhardToneMapping>()]: ReinhardToneMappingNode,
	[$type<CineonToneMapping>()]: OptimizedCineonToneMappingNode,
	[$type<ACESFilmicToneMapping>()]: ACESFilmicToneMappingNode,
	[$type<AgXToneMapping>()]: AGXToneMappingNode,
};

class ToneMappingNode extends TempNode {
	public toneMapping: any;
	public exposureNode: any;
	public colorNode: any;

	public constructor(toneMapping = NoToneMapping, exposureNode = toneMappingExposure, colorNode = null) {
		super('vec3');
	...
	public getCacheKey(): string {
		...
		cacheKey = '{toneMapping:' + $type(this.toneMapping) + ',nodes:' + cacheKey + '}';
		return cacheKey;
	}

	public setup(builder): any {
		...
		const toneMappingNode = toneMappingLib[$type(toneMapping)];
...
export function toneMapping(mapping, exposure, color) {
