...
class SDFGeometryGenerator {
    ...
    public function generate(res:Int = 64, distFunc:String = 'float dist( vec3 p ){ return length(p) - 0.5; }', bounds:Float = 1):BufferGeometry {
        ...
        var maxTexSize:Int = renderer.capabilities.maxTextureSize;
        ...
        var mesh:Array<Array<Float>> = surfaceNet([res, res, res], function(x:Float, y:Float, z:Float) {
            x = (x + bounds) * (res / (bounds * 2));
            y = (y + bounds) * (res / (bounds * 2));
            z = (z + bounds) * (res / (bounds * 2));
            var p:Int = Std.int((x + (z % tilesX) * res) + y * w + (Math.floor(z / tilesX) * res * w));
            p *= 4;
    ...
    public function computeSDF(width:Int, height:Int, tilesX:Int, tilesY:Int, bounds:Float, shader:String):WebGLRenderTarget {
        ...
        while (currentTile++ < tiles) {
            ...
            var py:Float = (tileSize) / 2 + Math.floor(c / tilesX) * (tileSize) - height / 2;
            var compPlane:Mesh = new Mesh(geometry, new ShaderMaterial({
                uniforms: {
                    res: new Vector2(width, height),
                    tileNum: c / (tilesX * tilesY - 1),
                    bounds: bounds
                },
        ...
        scn.traverse(function(object:Dynamic) {
