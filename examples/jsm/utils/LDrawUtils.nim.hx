...
class LDrawUtils {
	...
	static function mergeObject(object:Dynamic):Dynamic {
		...
		object.traverse(c => {
			...
			if (Std.is(c, Mesh) || Std.is(c, LineSegments)) {
				...
				var elemSize = Std.is(c, Mesh) ? 3 : 2;

				var geometry = c.geometry.clone();
				var matrixIsInverted = c.matrixWorld.determinant() < 0;
				if (matrixIsInverted) {

					permuteAttribute(geometry.getAttribute('position'), elemSize);
					permuteAttribute(geometry.getAttribute('normal'), elemSize);

				}

				geometry.applyMatrix4(c.matrixWorld);

				if (Std.is(c, ConditionalLine)) {

					geometry.getAttribute('control0').applyMatrix4(c.matrixWorld);
					geometry.getAttribute('control1').applyMatrix4(c.matrixWorld);
					normalMatrix.getNormalMatrix(c.matrixWorld);
					geometry.getAttribute('direction').applyNormalMatrix(normalMatrix);

				}

				var geometries = Std.is(c, Mesh) ? meshGeometries : (Std.is(c, ConditionalLine) ? condLinesGeometries : linesGeometries);
				...
				if (Std.is(c.material, Array)) {
					...
					for (groupIndex in geometry.groups) {
						...
						var newGeometry = extractGroup(geometry, group, elemSize, Std.is(c, ConditionalLine));
