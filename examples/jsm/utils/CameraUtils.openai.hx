...
class CameraUtils {
     ...
     * @param bottomLeftCorner Vector3
     * @param bottomRightCorner Vector3
     * @param topLeftCorner Vector3
     * @param estimateViewFrustum Bool
     */
    public static function frameCorners(camera:PerspectiveCamera, bottomLeftCorner:Vector3, bottomRightCorner:Vector3, topLeftCorner:Vector3, estimateViewFrustum:Bool = false) {
        ...
        if (estimateViewFrustum) {
            ...
            camera.fov = Math.PI / 180 * Math.atan(
                (_vec.copy(pb).sub(pa).length() + _vec.copy(pc).sub(pa).length()) / _va.length()
            );
        }
    }
}