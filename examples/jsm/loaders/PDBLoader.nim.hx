...
class PDBLoader extends Loader {
	...
	public function parse(text:String) {
		...
		function parseBond(start:Int, length:Int, satom:Int, i:Int) {
			...
			var eatom = Std.parseInt(text.slice(i, start, start + length));
		...
		for (i in 0...lines.length) {
			...
			if (lines[i].slice(0, 4) == "ATOM" || lines[i].slice(0, 6) == "HETATM") {
				...
				var x = Std.parseFloat(lines[i].slice(30, 37));
				var y = Std.parseFloat(lines[i].slice(38, 45));
				var z = Std.parseFloat(lines[i].slice(46, 53));
				var index = Std.parseInt(lines[i].slice(6, 11)) - 1;

				var e = trim(lines[i].slice(76, 78)).toLowerCase();
				...
				var satom = Std.parseInt(lines[i].slice(6, 11));
