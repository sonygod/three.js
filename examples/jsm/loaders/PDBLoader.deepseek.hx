...
class PDBLoader extends Loader {
	...
	public function parse(text:String):Dynamic {
		...
		function parseBond(start:Int, length:Int, satom:Int, i:Int):Void {
			var eatom = parseInt(lines[i].slice(start, start + length));
			if (eatom != 0) {
		...
		for (i in 0...lines.length) {
			if (lines[i].slice(0, 4) == 'ATOM' || lines[i].slice(0, 6) == 'HETATM') {
				var x = parseFloat(lines[i].slice(30, 37));
				var y = parseFloat(lines[i].slice(38, 45));
				var z = parseFloat(lines[i].slice(46, 53));
				var index = parseInt(lines[i].slice(6, 11)) - 1;
				...
				var satom = parseInt(lines[i].slice(6, 11));
