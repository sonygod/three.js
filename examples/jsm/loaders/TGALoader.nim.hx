...
class TGALoader extends DataTextureLoader {
	...
	public function parse(buffer:Bytes) {
		...
		function tgaParse(use_rle:Bool, use_pal:Bool, header:Dynamic, offset:Int, data:Bytes) {
				...
				while (shift < pixel_total) {
					...
					if (c & 0x80) {
						...
						for (i in 0...count) {
							...
							pixel_data[shift + i] = data.get(offset++);
		...
		function tgaGetImageData8bits(imageData:Bytes, y_start:Int, y_step:Int, y_end:Int, x_start:Int, x_step:Int, x_end:Int, image:Bytes, palettes:Bytes) {
			...
			var color:Int, i = 0, x:Int, y:Int;
			var width = header.width;

			for (y in y_start...y_end by y_step) {

				for (x in x_start...x_end by x_step) {
