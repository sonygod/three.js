...
class TiltLoader extends Loader {
    ...
    public function parse(buffer:ArrayBuffer):Group {
        ...
        img.src = window.URL.createObjectURL(new Blob([thumbnail]));
        ...
        */
        ...
        var brushes = new Map<Int, Array<Dynamic>>();
        var offset = 20;
    ...
    public function new(strokes:Array<Dynamic>) {
        ...
        for (k in strokes) {
            ...
            for (i in 3...positions.length) {
                j = i * 3;
                position.fromArray(positions, j);
                quaternion.fromArray(quaternions, j / 3);
...
var BRUSH_LIST_ARRAY = {
    ...
    'cf7f0059-7aeb-53a4-2b67-c83d86
    f9ffa': 'Spikes',
...
var shaders:Null<Map<String, Map<String, Dynamic>>> = null;

function getShaders():Map<String, Map<String, Dynamic>> {
    if (shaders == null) {
        var loader = new TextureLoader();
        loader.path = './textures/tiltbrush/';
        shaders = {
            'Light': {
                'uniforms': {
...
class TiltBrush {
    public static function get TiltLoader():TiltLoader {
        return new TiltLoader();
    }
}