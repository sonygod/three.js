...
class PDBLoader extends Loader {
    ...
    public function load(url:String, onLoad:(geometry:Any) -> Void, onProgress:(event:Event) -> Void, onError:(event:Event) -> Void) {
    ...
    function parse(text:String):Any {
        ...
        function parseBond(start:Int, length:Int, satom:Int, i:Int) {
            var eatom:Int = parseInt(lines[i].slice(start, start + length));
            if (eatom != 0) {
        ...
        function buildGeometry():Any {
            ...
            for (i in 0..._bonds.length) {
                ...
                var startAtom:Array<Float> = _atomMap[start];
                var endAtom:Array<Float> = _atomMap[end];

                var x:Float = startAtom[0];
                var y:Float = startAtom[1];
                var z:Float = startAtom[2];
        ...
        for (i in 0...lines.length) {
            if (lines[i].slice(0, 4) == 'ATOM' || lines[i].slice(0, 6) == 'HETATM') {
                var x:Float = parseFloat(lines[i].slice(30, 37));
                var y:Float = parseFloat(lines[i].slice(38, 45));
                var z:Float = parseFloat(lines[i].slice(46, 53));
                var index:Int = parseInt(lines[i].slice(6, 11)) - 1;
                ...
                var atomData:Array<Float> = [x, y, z, CPK[e], capitalize(e)];

                atoms.push(atomData);
                _atomMap[index] = atomData;
            } else if (lines[i].slice(0, 6) == 'CONECT') {
                var satom:Int = parseInt(lines[i].slice(6, 11));
