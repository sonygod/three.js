import three.extras.core.BufferGeometry;
import three.extras.core.Group;
import three.extras.loaders.Loader;
import three.extras.loaders.FileLoader;
import three.extras.loaders.TextureLoader;
import three.extras.loaders.shaders.RawShaderMaterial;
import three.extras.loaders.shaders.MeshBasicMaterial;
import three.extras.math.Color;
import three.extras.math.DoubleSide;
import three.extras.math.Quaternion;
import three.extras.math.Vector3;
import three.extras.objects.Mesh;
import three.extras.shaders.UniformsUtils;
import three.extras.textures.Texture;
import three.extras.utils.fflate.unzipSync;
import three.extras.utils.fflate.strFromU8;
...
class TiltLoader extends Loader {
	...
	public function parse(buffer:ArrayBuffer) {
		var group = new Group();
		...
		var brushes = new haxe.ds.StringMap<Array<Dynamic>>();
		var offset = 20;
		...
		for (i in 0...num_strokes) {
			...
			for (j in 0...positions.length) {
				positions[j] = data.getFloat32(offset + 0, true);
				positions[j + 1] = data.getFloat32(offset + 4, true);
				positions[j + 2] = data.getFloat32(offset + 8, true);
				quaternions[j] = data.getFloat32(offset + 12, true);
				quaternions[j + 1] = data.getFloat32(offset + 16, true);
				quaternions[j + 2] = data.getFloat32(offset + 20, true);
				quaternions[j + 3] = data.getFloat32(offset + 24, true);
		...
		shaders = {
			'Light': {
				uniforms: UniformsUtils.merge([
					UniformsUtils.clone(THREE.ShaderLib.basic.uniforms),
					{
...
function getMaterial(GUID:String) {
