...
class TiltLoader extends Loader {
	...
	public function parse(buffer:haxe.io.Bytes):Group {
		var group = new Group();
		// https://docs.google.com/document/d/11ZsHozYn9FnWG7y3s3WAyKIACfbfwb4PbaS8cZ_xjvo/edit#

		var zip = FFlate.unzipSync(new haxe.io.Bytes(buffer.slice(16)));

		/*
		var thumbnail = zip["thumbnail.png"].buffer;
		...
		*/
		...
		var brushes = new haxe.ds.StringMap<Array<Array<Float>>>();
		...
		for (i in 0...num_strokes) {
			...
			for (j in 0...positions.length / 3, k in 0...quaternions.length / 4) {

				positions[j * 3 + 0] = data.getFloat32(offset + 0, true);
				positions[j * 3 + 1] = data.getFloat32(offset + 4, true);
				positions[j * 3 + 2] = data.getFloat32(offset + 8, true);

				quaternions[k * 4 + 0] = data.getFloat32(offset + 12, true);
				quaternions[k * 4 + 1] = data.getFloat32(offset + 16, true);
				quaternions[k * 4 + 2] = data.getFloat32(offset + 20, true);
				quaternions[k * 4 + 3] = data.getFloat32(offset + 24, true);
...
class StrokeGeometry extends BufferGeometry {
	...
	public function new(strokes:Array<Array<Float>>) {
		super();
		...
		for (k in 0...strokes.length) {
			...
			for (i in 3...positions.length / 3, j in 4...quaternions.length / 4) {

				position.fromArray(positions, i * 3);
				quaternion.fromArray(quaternions, j * 4);
				...
				var p1 = i / (positions.length / 3);
				var p2 = (i - 3) / (positions.length / 3);
...
function getMaterial(GUID:String):Dynamic {
	...
	switch (name) {
		...
		default:
			return new MeshBasicMaterial({ vertexColors: true, side: Side.DoubleSide });
