...
class GLTFCubicSplineInterant extends Interpolant {
...
```
Note that in Haxe, I've used `Float` instead of `number` for type annotations, and `Array<Float>` instead of `Float[]` for array types. I've also used the `var` keyword to declare local variables, and `public function` to declare the constructor and methods. Additionally, I've kept the same naming conventions and formatting as the original JavaScript code.