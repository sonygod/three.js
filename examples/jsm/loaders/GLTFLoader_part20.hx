...
class GLTFBinaryExtension {
    ...
    public function new(data:Bytes) {
        ...
        var textDecoder = new Utf8();

        var header = {
            magic: textDecoder.decode(Bytes.ofString(data.toString().substr(0, 4))),
        ...
        while (chunkIndex < chunkContentsLength) {
            ...
            if (chunkType == BINARY_EXTENSION_CHUNK_TYPES.JSON) {
                var contentArray = Bytes.ofData(data.getBytes(BINARY_EXTENSION_HEADER_LENGTH + chunkIndex, chunkLength));
