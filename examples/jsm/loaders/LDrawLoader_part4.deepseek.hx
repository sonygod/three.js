class LDrawParsedCache {
	...
	public function cloneResult(original:Dynamic):Dynamic {
		...
		result.faces = original.faces.map(function(face) {
			return {
				...
				vertices: face.vertices.map(function(v) { return v.clone(); }),
				normals: face.normals.map(function() { return null; }),
				faceNormal: null
			};
		});
		result.conditionalSegments = original.conditionalSegments.map(function(face) {
			return {
				...
				material: face.material,
				vertices: face.vertices.map(function(v) { return v.clone(); }),
				controlPoints: face.controlPoints.map(function(v) { return v.clone(); })
			};
		});
	...
	public function fetchData(fileName:String):Promise<String> {
		...
		throw 'LDrawLoader: Subobject "' + fileName + '" could not be loaded.';
	}
	...
	public function parse(text:String, fileName:String = null):Dynamic {
		...
		var subobjects = [];
		var materials = {};
		var getLocalMaterial = function(colorCode:String) {
	...
	public function ensureDataLoaded(fileName:String):Promise<Void> {
		...
		if (!this._cache.exists(key)) {
			this._cache.set(key, this.fetchData(fileName).then(function(text) {
