...
class TDSLoader extends Loader {
	...
	public var materials:Map<String, MeshPhongMaterial>;
	 ...
	 */
	public function parse( arraybuffer:haxe.io.Bytes, path:String ):Group {
	 ...
	 * @param {String} path Path for external resources.
	 */
	public function readFile( arraybuffer:haxe.io.Bytes, path:String ):Void {
		var data = new DataView( arraybuffer.buffer );
	...
	public function readMaterialEntry( chunk:Chunk, path:String ):Void {
		...
		while( next != null ) {
			if( next.id == MAT_NAME ) {
				...
				material.side = three.constants.DoubleSide;
				this.debugMessage( '   DoubleSided' );
			} else if( next.id == MAT_ADDITIVE ) {
				this.debugMessage( '   Additive Blending' );
				material.blending = three.constants.AdditiveBlending;
				...
				var shininess = this.readPercentage( next );
				material.shininess = shininess * 100;
				this.debugMessage( '   Shininess : ' + shininess );
			} else if( next.id == MAT_TRANSPARENCY ) {
				var transparency = this.readPercentage( next );
	 ...
	 */
	public function readFaceArray( chunk:Chunk, mesh:Mesh ):Void {
		var faces = chunk.readWord();
		...
		while( ! chunk.endOfChunk ) {
			...
			if( subchunk.id == MSH_MAT_GROUP ) {
				...
				var group = this.readMaterialGroup( subchunk );
				var count = group.index.length * 3; // assuming successive indices
				mesh.geometry.addGroup( start, count, materialIndex );
				start += count;
				materialIndex++;
				var material = this.materials.get( group.name );
				if( Std.is( mesh.material, Array ) == false ) mesh.material = new Array<Dynamic>();
				if( material != null ) {
	 ...
	 */
	public function readMap( chunk:Chunk, path:String ):three.textures.Texture {
		var next = chunk.readChunk();
		var texture = null;
		var loader = new TextureLoader( this.manager );
		loader.setPath( this.resourcePath != null ? this.resourcePath : path ).setCrossOrigin( this.crossOrigin );
		while( next != null ) {
	 ...
	 */
	public function readMaterialGroup( chunk:Chunk ):{ name:String, index:Array<Int> } {
		var name = chunk.readString();
...
class Chunk {
	...
	public var debugMessage:Dynamic;

	public function new( data:DataView, position:Int, debugMessage:Dynamic ) {
		this.data = data;
		// the offset to the begin of this chunk
		...
		if( Std.is( this.debugMessage, Function ) ) {
	...
	public function get endOfChunk():Bool {
...
const M3DMAGIC:Int = 0x4D4D;
// const SMAGIC = 0x2D2D;
// const LMAGIC = 0x2D3D;
const MLIBMAGIC:Int = 0x3DAA;
// const MATMAGIC = 0x3DFF;
const CMAGIC:Int = 0xC23D;
const M3D_VERSION:Int = 0x0002;
// const M3D_KFVERSION = 0x0005;
const COLOR_F:Int = 0x0010;
const COLOR_24:Int = 0x0011;
const LIN_COLOR_24:Int = 0x0012;
const LIN_COLOR_F:Int = 0x0013;
const INT_PERCENTAGE:Int = 0x0030;
const FLOAT_PERCENTAGE:Int = 0x0031;
const MDATA:Int = 0x3D3D;
const MESH_VERSION:Int = 0x3D3E;
const MASTER_SCALE:Int = 0x0100;
...
const MAT_ENTRY:Int = 0xAFFF;
const MAT_NAME:Int = 0xA000;
const MAT_AMBIENT:Int = 0xA010;
const MAT_DIFFUSE:Int = 0xA020;
const MAT_SPECULAR:Int = 0xA030;
const MAT_SHININESS:Int = 0xA040;
// const MAT_SHIN2PCT = 0xA041;
const MAT_TRANSPARENCY:Int = 0xA050;
...
const MAT_TWO_SIDE:Int = 0xA081;
// const MAT_DECAL = 0xA082;
const MAT_ADDITIVE:Int = 0xA083;
const MAT_WIRE:Int = 0xA085;
// const MAT_FACEMAP = 0xA088;
// const MAT_TRANSFALLOFF_IN = 0xA08A;
// const MAT_PHONGSOFT = 0xA08C;
// const MAT_WIREABS = 0xA08E;
const MAT_WIRE_SIZE:Int = 0xA087;
const MAT_TEXMAP:Int = 0xA200;
...
const MAT_MAPNAME:Int = 0xA300;
// const MAT_MAP_TILING = 0xA351;
// const MAT_MAP_TEXBLUR = 0xA353;
const MAT_MAP_USCALE:Int = 0xA354;
const MAT_MAP_VSCALE:Int = 0xA356;
const MAT_MAP_UOFFSET:Int = 0xA358;
const MAT_MAP_VOFFSET:Int = 0xA35A;
