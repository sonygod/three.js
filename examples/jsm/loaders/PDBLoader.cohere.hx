...
class PDBLoader {
    ...
    public function parse(text:String):Dynamic {
        ...
        function parseBond(start:Int, length:Int, satom:Int, i:Int):Void {
            var eatom = Std.parseInt(lines[i].substr(start, length));
            if (eatom > 0) {
        ...
        function buildGeometry():Dynamic {
            ...
            for (i in 0..._bonds.length) {
                ...
                var startAtom = _atomMap.get(start);
                var endAtom = _atomMap.get(end);
        ...
        var _bonds = [];
        var _bhash = new haxe.ds.StringMap();
        var _atomMap = new haxe.ds.IntMap();
        ...
        for (i in 0...lines.length) {
            ...
            if (line.substr(0, 4) == 'ATOM' || line.substr(0, 6) == 'HETATM') {
                var x = Std.parseFloat(line.substr(30, 7));
                var y = Std.parseFloat(line.substr(38, 7));
                var z = Std.parseFloat(line.substr(46, 7));
                var index = Std.parseInt(line.substr(6, 5)) - 1;

                var e = trim(line.substr(76, 2)).toLowerCase();

                if (e == '') {
                    e = trim(line.substr(12, 2)).toLowerCase();
                }

                var atomData = [x, y, z, CPK.get(e), capitalize(e)];

                atoms.push(atomData);
                _atomMap.set(index, atomData);
            } else if (line.substr(0, 6) == 'CONECT') {
                var satom = Std.parseInt(line.substr(6, 5));
