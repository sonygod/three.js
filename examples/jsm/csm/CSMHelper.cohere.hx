...
class CSMHelper extends Group {
    ...
    var frustumLines: LineSegments;
    var cascadeLines: Array<Box3Helper>;
    var cascadePlanes: Array<Mesh>;
    var shadowLines: Array<Group>;

    public function new(csm: CSM) {
        super();
        this.csm = csm;
    ...
    public function update() {
        ...
        while (cascadeLines.length > cascades) {
            this.remove(Array.pop(cascadeLines));
            this.remove(Array.pop(cascadePlanes));
            this.remove(Array.pop(shadowLines));
        }

        while (cascadeLines.length < cascades) {
            var cascadeLine = new Box3Helper(new Box3(), 0xffffff);
            var planeMat = new MeshBasicMaterial({ transparent: true, opacity: 0.1, depthWrite: false, side: DoubleSide.DOUBLE_SIDED });
