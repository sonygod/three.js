...
class CSMHelper extends Group {
    ...
    public function update():Void {
        ...
        var cascades:Array<Dynamic> = csm.cascades;
        ...
        position.copyFrom(camera.position);
        quaternion.copyFrom(camera.quaternion);
        scale.copyFrom(camera.scale);
        updateMatrixWorld(true);

        while (cascadeLines.length > cascades.length) {
            remove(cascadeLines.pop());
            remove(cascadePlanes.pop());
            remove(shadowLines.pop());
        }

        while (cascadeLines.length < cascades.length) {
            var cascadeLine:Dynamic = new Box3Helper(new Box3(), 0xffffff);
        ...
        for (i in 0...cascades.length) {
            ...
            var cascadePlane:Dynamic = cascadePlanes[i];
            var shadowLineGroup:Dynamic = shadowLines[i];
            var shadowLine:Box3Helper = shadowLineGroup.children[0];
    ...
    public function dispose():Void {
        ...
        var cascades:Array<Dynamic> = this.csm.cascades;
