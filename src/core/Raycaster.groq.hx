...
class Raycaster {
    ...
    public function setFromCamera(coords:Array<Float>, camera:Dynamic):Void {
        if (camera.isPerspectiveCamera) {
            this.ray.origin.setFromMatrixPosition(camera.matrixWorld);
            this.ray.direction.set(coords[0], coords[1], 0.5).unproject(camera).sub(this.ray.origin).normalize();
            this.camera = camera;
        } else if (camera.isOrthographicCamera) {
            this.ray.origin.set(coords[0], coords[1], (camera.near + camera.far) / (camera.near - camera.far)).unproject(camera);
