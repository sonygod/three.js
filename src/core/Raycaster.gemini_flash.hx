...
class Raycaster {
	...
	public function set(origin:Dynamic, direction:Dynamic) {
		// direction is assumed to be normalized (for accurate distance calculations)
	...
	public function setFromCamera(coords:Dynamic, camera:Dynamic) {
		if (Std.isOfType(camera, three.cameras.PerspectiveCamera)) {
			this.ray.origin.setFromMatrixPosition(camera.matrixWorld);
			this.ray.direction.set(coords.x, coords.y, 0.5).unproject(camera).sub(this.ray.origin).normalize();
			this.camera = camera;
		} else if (Std.isOfType(camera, three.cameras.OrthographicCamera)) {
			this.ray.origin.set(coords.x, coords.y, (camera.near + camera.far) / (camera.near - camera.far)).unproject(camera); // set origin in plane of camera
			this.ray.direction.set(0, 0, - 1).transformDirection(camera.matrixWorld);
			this.camera = camera;
		} else {
			Sys.println('THREE.Raycaster: Unsupported camera type: ' + camera.type);
		}
	}
