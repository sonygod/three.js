...
class MaterialLoader extends Loader {
    ...
    public function parse(json:Dynamic):Material {
        ...
        function getTexture(name:String):Dynamic {
            if (!textures.exists(name)) {
                haxe.Log.trace('THREE.MaterialLoader: Undefined texture ' + name);
            }
        ...
        if (json.uniforms != null) {
            for (name in Reflect.fields(json.uniforms)) {
                var uniform = Reflect.field(json.uniforms, name);
        ...
        if (json.extensions != null) {
            for (key in Reflect.fields(json.extensions)) {
                material.extensions[key] = Reflect.field(json.extensions, key);
            }
        }
    ...
    public static function createMaterialFromType(type:String):Material {
        var materialLib:Map<String, Class<Material>> = [
