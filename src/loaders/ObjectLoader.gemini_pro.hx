...
class ObjectLoader extends Loader {
	...
	public function parse(json:Dynamic, onLoad:Dynamic->Void):Dynamic {
		...
		if (onLoad != null) {
			var hasImages = false;
			for (uuid in images) {
				if (images[uuid].data is HTMLImageElement) {
					hasImages = true;
					break;
				}
			}
	...
	public function parseAsync(json:Dynamic):Promise<Dynamic> {
		var animations = this.parseAnimations(json.animations);
		var shapes = this.parseShapes(json.shapes);
		var geometries = this.parseGeometries(json.geometries, shapes);
	...
	public function parseImages(json:Dynamic, onLoad:Dynamic->Void):Dynamic {
		...
		function deserializeImage(image:Dynamic):Dynamic {
			if (Std.is(image, String)) {
				var url = image;
				var path = (new EReg("^((\\/\\/)|([a-z]+:(\\/\\/)?))", "i")).match(url) != null ? url : scope.resourcePath + url;
	...
	public function parseObject(data:Dynamic, geometries:Dynamic, materials:Dynamic, textures:Dynamic, animations:Dynamic):Dynamic {
		...
		var geometry:Dynamic, material:Dynamic;
		switch(data.type) {
			case "Scene":
				object = new Scene();
			...
			case "PerspectiveCamera":
				...
				if (data.view != null) object.view = Reflect.copy(data.view);
				break;
...
class ObjectLoader {
	...
	public static function loadAsync(url:String, onProgress:Dynamic->Void):Promise<Dynamic> {
		var loader = new ObjectLoader();
		return loader.loadAsync(url, onProgress);
	}

	public static function parse(json:Dynamic, onLoad:Dynamic->Void):Dynamic {
		var loader = new ObjectLoader();
		return loader.parse(json, onLoad);
	}

	public static function parseAsync(json:Dynamic):Promise<Dynamic> {
		var loader = new ObjectLoader();
