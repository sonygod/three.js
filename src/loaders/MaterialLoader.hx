...
class MaterialLoader extends Loader {
    ...
    public function load(url:String, onLoad:(material:Material)->Void, onProgress:(progress:Int)->Void, onError:(error:Dynamic)->Void):Void {
    ...
    function parse(json:Dynamic):Material {
        ...
        if (json.uniforms != null) {
            for (uniform in json.uniforms) {
                ...
                switch (value.type) {
                    case 't':
                        material.uniforms[name].value = getTexture(value.value);
                        break;
                    case 'c':
                        material.uniforms[name].value = new Color().setHex(value.value);
                        break;
                    case 'v2':
                        material.uniforms[name].value = new Vector2().fromArray(value.value);
                        break;
                    case 'v3':
                        material.uniforms[name].value = new Vector3().fromArray(value.value);
                        break;
                    case 'v4':
                        material.uniforms[name].value = new Vector4().fromArray(value.value);
                        break;
                    case 'm3':
                        material.uniforms[name].value = new Matrix3().fromArray(value.value);
                        break;
                    case 'm4':
                        material.uniforms[name].value = new Matrix4().fromArray(value.value);
                        break;
                    default:
                        material.uniforms[name].value = value.value;
                }
            }
        }
        ...
        if (json.normalScale != null) {
            var normalScale:Array<Dynamic> = json.normalScale;
    ...
    static function createMaterialFromType(type:String):Material {
        var materialLib:Map<String, Dynamic> = {
