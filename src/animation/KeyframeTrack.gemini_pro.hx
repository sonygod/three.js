...
class KeyframeTrack {
	...
	public function new(name:String, times:Array<Float>, values:Array<Float>, interpolation:Int = InterpolateLinear) {
		if (name == null) {
		...
		this.times = AnimationUtils.convertArray(times, Float);
		this.values = AnimationUtils.convertArray(values, Float);
	...
	public function setInterpolation(interpolation:Int):KeyframeTrack {
		...
		if (factoryMethod == null) {
			...
			if (this.createInterpolant == null) {
				if (interpolation != KeyframeTrack.DefaultInterpolation) {
					this.setInterpolation(KeyframeTrack.DefaultInterpolation);
				} else {
	...
	public function getValueSize():Float {
	...
	public function validate():Bool {
		...
		for (var i = 0; i < nKeys; i++) {
			...
			if (typeof currTime == 'number' && Math.isNaN(currTime)) {
	...
	public function clone():KeyframeTrack {
		...
		var TypedKeyframeTrack = this.getClass();
	...
	public static var TimeBufferType:Class<Float> = Float;
	public static var ValueBufferType:Class<Float> = Float;
