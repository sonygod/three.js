...
class AnimationAction {
    ...
    public function new(mixer:Dynamic, clip:Dynamic, localRoot:Dynamic = null, blendMode:Int = -1) {
        ...
        _blendMode = (blendMode == -1) ? clip.blendMode : blendMode;
    ...
    public function stopFading():AnimationAction {
        if (_weightInterpolant != null) {
            _mixer._takeBackControlInterpolant(_weightInterpolant);
    ...
    public function warp(startTimeScale:Float, endTimeScale:Float, duration:Float):AnimationAction {
        ...
        var interpolant:Null<Dynamic> = _timeScaleInterpolant;
    ...
    public function stopWarping():AnimationAction {
        if (_timeScaleInterpolant != null) {
            _mixer._takeBackControlInterpolant(_timeScaleInterpolant);
    ...
    private function _updateWeight(time:Float):Void {
        var weight:Float = 0;

        if (enabled) {
            weight = this.weight;
            var interpolant:Null<Dynamic> = _weightInterpolant;
        ...
        if (loop == LoopOnce) {
            ...
            if (time >= duration || time < 0) {
                ...
                if (pending <= 0) {
                    ...
                    if (pending == 1) {
                        _setEndings(deltaTime < 0, !deltaTime < 0, pingPong);
                    } else {
