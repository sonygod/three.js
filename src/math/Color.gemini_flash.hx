...
import three.math.ColorManagement.LinearToSRGB;
...
class Color {
	...
	public function new(r:Dynamic, g:Dynamic = null, b:Dynamic = null) {
		this.set(r, g, b);
	}
	...
	public function setStyle(style:String, colorSpace:Int = SRGBColorSpace):Color {
		...
		if (m != null) {
			...
			switch (name) {
				case "rgb":
				case "rgba":
					color = RegExp.match(components, "^\\s*(\\d+)\\s*,\\s*(\\d+)\\s*,\\s*(\\d+)\\s*(?:,\\s*(\\d*\\.?\\d+)\\s*)?$");
					if (color != null) {
						handleAlpha(color[4]);
						return this.setRGB(
				...
				case "hsl":
				case "hsla":
					color = RegExp.match(components, "^\\s*(\\d*\\.?\\d+)\\s*,\\s*(\\d*\\.?\\d+)\\%\\s*,\\s*(\\d*\\.?\\d+)\\%\\s*(?:,\\s*(\\d*\\.?\\d+)\\s*)?$");
					if (color != null) {
						handleAlpha(color[4]);
						return this.setHSL(
		...
		} else if (m = RegExp.match(style, "^\#([A-Fa-f\\d]+)$")) {
			var hex = m[1];
			var size = hex.length;
			if (size == 3) {
				return this.setRGB(
	...
	public function setColorName(style:String, colorSpace:Int = SRGBColorSpace):Color {
		var hex = Color.NAMES[style.toLowerCase()];
		if (hex != null) {
	...
	public function iterator():Iterator<Float> {
		return new _ColorIterator(this.r, this.g, this.b);
	}
	...
	static private function handleAlpha(string:String):Void {
		...
		if (Std.parseFloat(string) < 1) {
			Sys.println("THREE.Color: Alpha component of " + style + " will be ignored.");
		}
	}
	...
	private static var _colorKeywords = {
...
class _ColorIterator implements Iterator<Float> {
	public var r:Float;
	public var g:Float;
	public var b:Float;
