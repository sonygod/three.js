...
class Vector4 {
    ...
    public function setAxisAngleFromRotationMatrix(m:Array<Float>):Vector4 {
        // implement the algorithm from http://www.euclideanspace.com/maths/geometry/rotations/conversions/matrixToAngle/index.htm
        var angle:Float;
        var x:Float, y:Float, z:Float; // variables for result
        var epsilon:Float = 0.01, epsilon2:Float = 0.1; // margin to allow for rounding errors
        var te:Array<Float> = m,
            m11:Float = te[0], m12:Float = te[4], m13:Float = te[8],
        ...
        var s:Float = Math.sqrt((m32 - m23) * (m32 - m23) + (m13 - m31) * (m13 - m31) + (m21 - m12) * (m21 - m12)); // used to normalize
    ...
    public function iterator():Iterator<Float> {
        return new Iterator<Float>([this.x, this.y, this.z, this.w]);
    }
}