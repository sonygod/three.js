...
class Spherical {
    ...
    public var phi:Float; // polar angle
    public var theta:Float; // azimuthal angle
...
```
Note that I've used the `three.math` package, assuming that's where this class belongs. I've also replaced `export` with the Haxe equivalent, which is simply making the class public.