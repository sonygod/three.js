...
class Color {
	...
	public function setHex(hex:Int, colorSpace:Int = 0):Color {
		hex = Math.floor(hex);
	...
	public function setRGB(r:Float, g:Float, b:Float, colorSpace:Int = 0):Color {
		this.r = r;
		this.g = g;
		this.b = b;
		// ColorManagement.toWorkingColorSpace( this, colorSpace );
	...
	public function setHSL(h:Float, s:Float, l:Float, colorSpace:Int = 0):Color {
		h = Math.fmod(h, 1);
		s = Math.clamp(s, 0, 1);
		l = Math.clamp(l, 0, 1);
		if (s == 0) {
	...
	public function setStyle(style:String, colorSpace:Int = 0):Color {
		var m = style.match(/(\\w+)\\(([^\\)]*)\\)/);
		if (m != null) {
			var color:Dynamic;
			var name = m[1];
	...
	public function setColorName(style:String, colorSpace:Int = 0):Color {
		var hex = colorKeywords[style.toLowerCase()];
		if (hex != null) {
			// red
	...
	public function getHex(colorSpace:Int = 0):Int {
		// ColorManagement.fromWorkingColorSpace( _color.copy( this ), colorSpace );
		return Math.round(Math.clamp(this.r * 255, 0, 255)) * 65536 + Math.round(Math.clamp(this.g * 255, 0, 255)) * 256 + Math.round(Math.clamp(this.b * 255, 0, 255));
	}

	public function getHexString(colorSpace:Int = 0):String {
		return ("000000" + this.getHex(colorSpace).toString(16)).slice(-6);
	}

	public function getHSL(target:Dynamic, colorSpace:Int = 0):Dynamic {
	...
	public function getRGB(target:Dynamic, colorSpace:Int = 0):Dynamic {
		// ColorManagement.fromWorkingColorSpace( _color.copy( this ), colorSpace );
		target.r = this.r;
		target.g = this.g;
		target.b = this.b;
	...
	public function getStyle(colorSpace:Int = 0):String {
		// ColorManagement.fromWorkingColorSpace( _color.copy( this ), colorSpace );
		var r = this.r, g = this.g, b = this.b;
		if (colorSpace != 0) {
	...
	public function lerpHSL(color:Color, alpha:Float):Color {
		this.getHSL(_hslA);
		color.getHSL(_hslB);
		var h = Math.lerp(_hslA.h, _hslB.h, alpha);
		var s = Math.lerp(_hslA.s, _hslB.s, alpha);
		var l = Math.lerp(_hslA.l, _hslB.l, alpha);
		this.setHSL(h, s, l);
		return this;
	}
	...
	private static var colorKeywords:Map<String, Int> = new Map<String, Int>([
		"aliceblue", 0xF0F8FF,
	...
	private static var _hslA:Dynamic = { h: 0, s: 0, l: 0 };
	private static var _hslB:Dynamic = { h: 0, s: 0, l: 0 };

	private static class _Iterator implements Iterator<Float> {
...
private static function SRGBToLinear(c:Float):Float {
	return c <= 0.04045 ? c / 12.92 : Math.pow((c + 0.055) / 1.055, 2.4);
}

private static function LinearToSRGB(c:Float):Float {
