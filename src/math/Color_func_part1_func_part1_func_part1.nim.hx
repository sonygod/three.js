import ColorManagement.clamp;
import ColorManagement.euclideanModulo;
import ColorManagement.lerp;
...
class Color {
  ...
  public function setHex(hex:Int, colorSpace:SRGBColorSpace):Color {
    hex = Std.int(Math.floor(hex));
  ...
  public function setRGB(r:Float, g:Float, b:Float, colorSpace:SRGBColorSpace):Color {
    this.r = r;
    this.g = g;
    this.b = b;
    ColorManagement.toWorkingColorSpace(this, colorSpace);
  ...
  public function setHSL(h:Float, s:Float, l:Float, colorSpace:SRGBColorSpace):Color {
  ...
  public function setStyle(style:String, colorSpace:SRGBColorSpace):Color {
    ...
    if (m = ~/^(\w+)\(([^\)]*)\)/.exec(style)) {
      ...
      switch (name) {
        ...
        case 'rgba':
          if (color = ~/^\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*(\d*\.?\d+)\s*)?$/.exec(components)) {
            handleAlpha(color[4]);
            return this.setRGB(Math.min(255, Std.parseInt(color[1])) / 255, Math.min(255, Std.parseInt(color[2])) / 255, Math.min(255, Std.parseInt(color[3])) / 255, colorSpace);
          }
          if (color = ~/^\s*(\d+)\%\s*,\s*(\d+)\%\s*,\s*(\d+)\%\s*(?:,\s*(\d*\.?\d+)\s*)?$/.exec(components)) {
            handleAlpha(color[4]);
            return this.setRGB(Math.min(100, Std.parseInt(color[1])) / 100, Math.min(100, Std.parseInt(color[2])) / 100, Math.min(100, Std.parseInt(color[3])) / 100, colorSpace);
          }
          break;
        case 'hsl':
        case 'hsla':
          if (color = ~/^\s*(\d*\.?\d+)\s*,\s*(\d*\.?\d+)\%\s*,\s*(\d*\.?\d+)\%\s*(?:,\s*(\d*\.?\d+)\s*)?$/.exec(components)) {
            handleAlpha(color[4]);
            return this.setHSL(Std.parseFloat(color[1]) / 360, Std.parseFloat(color[2]) / 100, Std.parseFloat(color[3]) / 100, colorSpace);
          }
          break;
        default:
      ...
      var size = hex.length;
      if (size == 3) {
        return this.setRGB(Std.parseInt(hex.charAt(0), 16) / 15, Std.parseInt(hex.charAt(1), 16) / 15, Std.parseInt(hex.charAt(2), 16) / 15, colorSpace);
      } else if (size == 6) {
  ...
  public function setColorName(style:String, colorSpace:SRGBColorSpace):Color {
  ...
  public function getHex(colorSpace:SRGBColorSpace):Int {
  ...
  public function getHexString(colorSpace:SRGBColorSpace):String {
    return ('000000' + this.getHex(colorSpace).toString(16)).slice(-6);
  }

  public function getHSL(target:Color, colorSpace:SRGBColorSpace):Color {
    var _color = new Color();
    ColorManagement.fromWorkingColorSpace(_color.copy(this), colorSpace);
  ...
  public function getRGB(target:Color, colorSpace:SRGBColorSpace):Color {
    var _color = new Color();
    ColorManagement.fromWorkingColorSpace(_color.copy(this), colorSpace);
    target.r = _color.r;
    target.g = _color.g;
  ...
  public function getStyle(colorSpace:SRGBColorSpace):String {
    var _color = new Color();
    ColorManagement.fromWorkingColorSpace(_color.copy(this), colorSpace);
    var r = _color.r, g = _color.g, b = _color.b;
  ...
  public function fromArray(array:Array<Float>, offset:Int):Color {
    this.r = array[offset];
...
class ColorManagement {
  ...
  public static function euclideanModulo(value:Float, max:Float):Float {
    return (value % max + max) % max;
  }

  public static function lerp(value1:Float, value2:Float, alpha:Float):Float {
  ...
  public static function toWorkingColorSpace(color:Color, colorSpace:SRGBColorSpace):Color {
    if (colorSpace == SRGBColorSpace) {
  ...
  public static function fromWorkingColorSpace(color:Color, colorSpace:SRGBColorSpace):Color {
    if (colorSpace == SRGBColorSpace) {
