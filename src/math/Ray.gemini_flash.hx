...
class Ray {
	...
	static var _segCenter:Vector3 = new Vector3();
	static var _segDir:Vector3 = new Vector3();
	static var _diff:Vector3 = new Vector3();

	static var _edge1:Vector3 = new Vector3();
	static var _edge2:Vector3 = new Vector3();
	...
	function isNaN(f:Float):Bool {
		return !Math.isFinite(f) && !Math.isNaN(f);
	}
	...
	public function intersectTriangle(a:Vector3, b:Vector3, c:Vector3, backfaceCulling:Bool, target:Vector3):Vector3 {
		...
		var DdQxE2:Float = sign * this.direction.dot(_edge2.crossVectors(_diff, _edge2));
		if (DdQxE2 < 0) {
			return null;
		}
