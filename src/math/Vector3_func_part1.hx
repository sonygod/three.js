...
class Vector3 {
    ...
    public function applyAxisAngle(axis:Vector3, angle:Float):Vector3 {
        return applyQuaternion(new Quaternion().setFromAxisAngle(axis, angle));
    }
    ...
    public function applyMatrix3(m:Matrix3):Vector3 {
        ...
        const y = this.y;
        const z = this.z;
        const e:Array<Float> = m.elements;
    ...
    public function projectOnPlane(planeNormal:Vector3):Vector3 {
        const v = _vector.copy(this).projectOnVector(planeNormal);
        return this.sub(v);
    }
    ...
    public function setFromSpherical(s:SphericalCoords):Vector3 {
    ...
    public function setFromCylindrical(c:CylindricalCoords):Vector3 {
...
class Vector3Iterator implements Iterator<Float> {
    ...
    public function next():Float {
        return switch (this.index++) {
            case 0: this.vector.x;
            case 1: this.vector.y;
            case 2: this.vector.z;
            default: throw "Error: next() called on exhausted iterator";
        }
    }
}