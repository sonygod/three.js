...
class Box3 {
    ...
    public function expandByObject(object:Object3D, precise:Bool = false):Box3 {
        ...
        if (geometry != null) {
            ...
            if (precise && positionAttribute != null && !(object is InstancedMesh)) {
                for (i in 0...positionAttribute.count) {
                    var point = new Vector3().fromBufferAttribute(positionAttribute, i);
                    point.applyMatrix4(object.matrixWorld);
                    this.expandByPoint(point);
                }
            } else {
                var box = (object.boundingBox != null) ? object.boundingBox : geometry.boundingBox;
                box.copy(box).applyMatrix4(object.matrixWorld);
                this.union(box);
            }
        }
    ...
    public function intersectsTriangle(triangle:Triangle):Bool {
        ...
        _extents.subVectors(_center, this.min);
...
private var _points:Array<Vector3> = [new Vector3(), new Vector3(), new Vector3(), new Vector3(), new Vector3(), new Vector3(), new Vector3(), new Vector3()];
...
private function satForAxes(axes:Array<Float>, v0:Vector3, v1:Vector3, v2:Vector3, extents:Vector3):Bool {
