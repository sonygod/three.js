...
class Color {
    ...
    public function set(color:Dynamic):Color {
        if (Std.is(color, Color)) {
            this.copy(color);
        } else if (Std.is(color, Int)) {
            this.setHex(color);
    ...
    public function setStyle(style:String, colorSpace:String = null):Color {
        var m = new EReg("^\\#([A-Fa-f0-9]+)$", "").match(style);
        ...
        if (m != null) {
            ...
            if (size == 3) {
                this.r = Std.parseInt("0x" + hex.charAt(0) + hex.charAt(0)) / 255.0;
                this.g = Std.parseInt("0x" + hex.charAt(1) + hex.charAt(1)) / 255.0;
                this.b = Std.parseInt("0x" + hex.charAt(2) + hex.charAt(2)) / 255.0;
                return this;
            }

            if (size == 6) {
                this.setHex(Std.parseInt("0x" + hex), colorSpace);
                return this;
            }
        }
