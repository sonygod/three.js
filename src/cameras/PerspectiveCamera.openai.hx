...
class PerspectiveCamera extends Camera {
    ...
    public function getViewBounds(distance:Float, minTarget:Vector2, maxTarget:Vector2):Void {
        this._v3.set(-1, -1, 0.5).applyMatrix4(this.projectionMatrixInverse);
        minTarget.set(this._v3.x, this._v3.y).multiplyScalar(-distance / this._v3.z);
        this._v3.set(1, 1, 0.5).applyMatrix4(this.projectionMatrixInverse);
        maxTarget.set(this._v3.x, this._v3.y).multiplyScalar(-distance / this._v3.z);
    }

    public function getViewSize(distance:Float, target:Vector2):Vector2 {
        this.getViewBounds(distance, this._minTarget, this._maxTarget);
        return target.subVectors(this._maxTarget, this._minTarget);
    }
