...
class Sprite extends Object3D {
    ...
    private var _intersectPoint:Vector3 = new Vector3();
    ...
    private function initGeometry():BufferGeometry {
        if (_geometry == null) {
    ...
    public function raycast(raycaster:Raycaster, intersects:Array<Intersection>) {
        ...
        _uvB.set(1, 0);
        _uvC.set(1, 1);
        var intersect:Intersection = raycaster.ray.intersectTriangle(_vA, _vB, _vC, false, _intersectPoint);
        if (intersect == null) {
            transformVertex(_vB.set(-0.5, 0.5, 0), _mvPosition, center, _worldScale, sin, cos);
            _uvB.set(0, 1);
    ...
    public function copy(source:Sprite, recursive:Bool = true):Sprite {
