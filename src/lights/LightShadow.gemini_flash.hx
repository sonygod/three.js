...
class LightShadow {
	...
	public function new(camera:three.Camera) {
		this.camera = camera;
		this.bias = 0;
		this.normalBias = 0;
		this.radius = 1;
	...
	public function toJSON():Dynamic {
		var object = new haxe.ds.StringMap();
		if (this.bias != 0) object.set("bias", this.bias);
		if (this.normalBias != 0) object.set("normalBias", this.normalBias);
		if (this.radius != 1) object.set("radius", this.radius);
		if (this.mapSize.x != 512 || this.mapSize.y != 512) object.set("mapSize", this.mapSize.toArray());
		object.set("camera", this.camera.toJSON(false).object);
		object.remove("camera.matrix");
		return object;
	}
}