...
var XYZ_TO_REC709 : Mat3 = new Mat3(
...
function Fresnel0ToIor(fresnel0 : Vec3) : Vec3 {
	...
	return (Vec3.one + sqrtF0) / (Vec3.one - sqrtF0);
}

// Conversion FO/IOR
function IorToFresnel0(transmittedIor : Vec3, incidentIor : Float) : Vec3 {
	return pow2((transmittedIor - Vec3.fromFloat(incidentIor)) / (transmittedIor + Vec3.fromFloat(incidentIor)));
}

// ior is a value between 1.0 and 3.0. 1.0 is air interface
function IorToFresnel0Scalar(transmittedIor : Float, incidentIor : Float) : Float {
...
function evalSensitivity(OPD : Float, shift : Vec3) : Vec3 {
	...
	var rgb : Vec3 = XYZ_TO_REC709.mult(xyz);
	return rgb;
}
...
function evalIridescence(outsideIOR : Float, eta2 : Float, cosTheta1 : Float, thinFilmThickness : Float, baseF0 : Vec3) : Vec3 {
	...
	var iridescenceIOR : Float = outsideIOR + (eta2 - outsideIOR) * smoothstep(0.0, 0.03, thinFilmThickness);
	...
	// First interface
	var R0 : Float = IorToFresnel0Scalar(iridescenceIOR, outsideIOR);
	...
	var Rs : Vec3 = pow2(T121) * R23 / (Vec3.one - R123);
	...
	return max(I, new Vec3(0.0, 0.0, 0.0));
}
