...
class TransmissionParsFragment {
    @:uniform var transmission:Float;
    @:uniform var thickness:Float;
    @:uniform var attenuationDistance:Float;
    @:uniform var attenuationColor:Vec3;

    #ifdef USE_TRANSMISSIONMAP
    @:uniform var transmissionMap:Texture;
    #end

    #ifdef USE_THICKNESSMAP
    @:uniform var thicknessMap:Texture;
    #end

    @:uniform var transmissionSamplerSize:Vec2;
    @:uniform var transmissionSamplerMap:Texture;

    @:uniform var modelMatrix:Mat4;
    ...
    function w2(a:Float):Float {
        return (1.0 / 6.0) * (a * (a * (-3.0 * a + 3.0) + 3.0));
    }
    ...
    function bicubic(tex:Texture, uv:Vec2, texelSize:Vec4, lod:Float):Vec4 {
        ...
        return g0(fuv.y) * (g0x * tex.get TX2(p0) + g1x * tex.get TX2(p1)) + g1(fuv.y) * (g0x * tex.get TX2(p2) + g1x * tex.get TX2(p3));
    }
    ...
    function getIBLVolumeRefraction(n:Vec3, v:Vec3, roughness:Float, diffuseColor:Vec3, specularColor:Vec3, specularF90:Float,
        position:Vec3, modelMatrix:Mat4, viewMatrix:Mat4, projMatrix:Mat4, dispersion:Float, ior:Float, thickness:Float,
