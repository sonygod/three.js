class PMREMGenerator {
    ...
    private lodPlanes: BufferGeometry[];
    private _sizeLods: number[];
    private _lodMax: number;
    private _cubeSize: number;

    constructor(renderer: WebGLRenderer, blurMaterial: ShaderMaterial) {
        this.renderer = renderer;
        this.blurMaterial = blurMaterial;
        const { lodPlanes, sizeLods } = this._createPlanes(this._lodMax);
        this.lodPlanes = lodPlanes;
        this._sizeLods = sizeLods;
    }

    private _createPlanes(lodMax: number) {
    ...
    private _createRenderTarget(width: number, height: number, params: WebGLRenderTargetOptions) {
    ...
    private _setViewport(target: WebGLRenderTarget, x: number, y: number, width: number, height: number) {
        target.viewport.set(x, y, width, height);
        target.scissor.set(x, y, width, height);
    }

    private _getBlurShader(lodMax: number, width: number, height: number) {
    ...
    private _getCommonVertexShader() {
        return `
            precision mediump float;
            precision mediump int;
