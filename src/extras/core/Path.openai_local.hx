...
class Path extends CurvePath {
    ...
    public function ellipse(aX:Float, aY:Float, xRadius:Float, yRadius:Float, aStartAngle:Float, aEndAngle:Float, aClockwise:Bool, aRotation:Float = 0):Path {
        var x0 = this.currentPoint.x;
        var y0 = this.currentPoint.y;
    ...
    public override function copy(source:CurvePath):Path {
        super.copy(source);
        this.currentPoint.copy((cast source:Path).currentPoint);
        return this;
    }
    ...
    public override function fromJSON(json:Dynamic):Path {
        super.fromJSON(json);
