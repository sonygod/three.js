...
class CameraHelper extends LineSegments {
	static _vector = new Vector3();
	static _camera = new Camera();
	...
	public function new(camera:Camera) {
		...
		this.camera = camera;
		if (camera.updateProjectionMatrix != null) camera.updateProjectionMatrix();
...
function setPoint(point:String, pointMap:Map<String,Array<Int>>, geometry:BufferGeometry, camera:Camera, x:Float, y:Float, z:Float) {
	CameraHelper._vector.set(x, y, z).unproject(camera);
