	...
	public function new(camera:Camera) {
		...
		function addPoint(id:String) {
			...
			if (pointMap.exists(id)) {
				...
				pointMap.set(id, [ (vertices.length / 3) - 1 ]);
...
function setPoint(point:String, pointMap:Map<String,Array<Int>>, geometry:BufferGeometry, camera:Camera, x:Float, y:Float, z:Float) {
	...
	var _vector = new Vector3(x, y, z);
	_vector.unproject(camera);

	if (pointMap.exists(point)) {

		var position = geometry.getAttribute("position");

		for (i in 0...pointMap.get(point).length) {

			position.setXYZ(pointMap.get(point)[i], _vector.x, _vector.y, _vector.z);
