...
class PolarGridHelper extends LineSegments {
    public function new(radius: Float = 10, sectors: Int = 16, rings: Int = 8, divisions: Int = 64, color1: Int = 0x444444, color2: Int = 0x888888) {
        super();
        var color1_ = new Color(color1);
        var color2_ = new Color(color2);
        ...
        if (sectors > 1) {
            for (var i:Int in 0...sectors) {
                ...
                var color:Color = (i & 1) > 0 ? color1_ : color2_;
        ...
        // create the rings
        for (var i:Int in 0...rings) {
            var color:Color = (i & 1) > 0 ? color1_ : color2_;

            var r:Float = radius - (radius / rings * i);

            for (var j:Int in 0...divisions) {
                // first vertex
