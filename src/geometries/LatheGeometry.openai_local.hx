...
class LatheGeometry extends BufferGeometry {
    ...
    public function new(points:Array<Vector2> = [new Vector2(0, -0.5), new Vector2(0.5, 0), new Vector2(0, 0.5)], segments:Int = 12, phiStart:Float = 0, phiLength:Float = Math.PI * 2) {
        ...
        for (i in 0...segments + 1) {
            ...
            for (j in 0...points.length) {
                ...
                // normal
                var x = initNormals[3 * j] * sin;
                var y = initNormals[3 * j + 1];
                var z = initNormals[3 * j] * cos;
    ...
    public override function copy(source:LatheGeometry):LatheGeometry {
        super.copy(source);
        this.parameters = haxe.DynamicAccess.createDynamicAccess(source.parameters);
        return this;
    }
