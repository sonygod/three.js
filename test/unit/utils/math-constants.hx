...
```
Note that in Haxe, we use `Math.POSITIVE_INFINITY` to represent infinity, and `Float` type for floating-point numbers. Also, we define a `MathConstants` class to hold the static constants, as Haxe does not have a direct equivalent to JavaScript's module exports.