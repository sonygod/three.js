class SmartComparer {
	...
	public function areEqual(val1:Dynamic, val2:Dynamic):Bool {
		...
		if (Reflect.hasField(val1, 'equals')) {
			if (Reflect.callMethod(val1, 'equals', [val2])) return true;
	...
	private function isFunction(value:Dynamic):Bool {
		...
		var tag = this.isObject(value) ? Type.typeof(value) : '';
	...
	private function compareArrays(val1:Dynamic, val2:Dynamic):Bool {
		var isArr1 = Type.typeof(val1) == 'Array';
		var isArr2 = Type.typeof(val2) == 'Array';
	...
	private function makeFail(msg:String, val1:Dynamic, val2:Dynamic):Bool {
		this._message = msg;
		if (arguments.length > 1) this._message += ' (' + val1 + ' vs ' + val2 + ')';
		return false;
	}

	private function addContext(msg:String):Bool {
		// There should already be a validation message. Add more context to it.
		this._message = this._message != null ? this._message : 'Error';
		this._message += ', at ' + msg;
		return false;
	}
