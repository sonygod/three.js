...
class RaycasterTests {
    ...
    public static function checkRayDirectionAgainstReferenceVector(rayDirection:Vector3, refVector:Vector3, assert:Dynamic) {
        assert.ok(Math.abs(refVector.x - rayDirection.x) <= Math.EPSILON && Math.abs(refVector.y - rayDirection.y) <= Math.EPSILON && Math.abs(refVector.z - rayDirection.z) <= Math.EPSILON, 'camera is pointing to the same direction as expected');
    }
        ...
        QUnit.module("Core", () => {
            QUnit.module("Raycaster", () => {
                ...
                QUnit.test("setFromCamera (Perspective)", (assert) => {
                    ...
                    assert.ok(rayDirection.x == 0 && rayDirection.y == 0 && rayDirection.z == -1, 'camera is looking straight to -z and so does the ray in the middle of the screen');
                ...
                QUnit.test("intersectObject", (assert) => {
                    ...
                    assert.ok(raycaster.intersectObject(objectsToCheck[0], false).length == 1, 'no recursive search should lead to one hit');

                    assert.ok(raycaster.intersectObject(objectsToCheck[0]).length == 3, 'recursive search should lead to three hits');

                    var intersections:Array<Dynamic> = raycaster.intersectObject(objectsToCheck[0]);
                ...
                QUnit.test("intersectObjects", (assert) => {
                    ...
                    assert.ok(raycaster.intersectObjects(objectsToCheck, false).length == 1, 'no recursive search should lead to one hit');

                    assert.ok(raycaster.intersectObjects(objectsToCheck).length == 3, 'recursive search should lead to three hits');

                    var intersections:Array<Dynamic> = raycaster.intersectObjects(objectsToCheck);
                    for (i in 0...intersections.length - 1) {
                        assert.ok(intersections[i].distance <= intersections[i + 1].distance, 'intersections are sorted');
                    }
                });
                ...
                QUnit.test("Line intersection threshold", (assert) => {
                    ...
                    assert.ok(raycaster.intersectObject(line).length == 0, 'no Line intersection with a not-large-enough threshold');

                    raycaster.params.Line.threshold = 2.001;
                    assert.ok(raycaster.intersectObject(line).length == 1, 'successful Line intersection with a large-enough threshold');
                });
                ...
                QUnit.test("Points intersection threshold", (assert) => {
                    ...
                    assert.ok(raycaster.intersectObject(points).length == 0, 'no Points intersection with a not-large-enough threshold');

                    raycaster.params.Points.threshold = 2.001;
                    assert.ok(raycaster.intersectObject(points).length == 1, 'successful Points intersection with a large-enough threshold');
                });
            });
        });
    }
