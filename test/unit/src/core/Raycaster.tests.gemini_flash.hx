...
class RaycasterTest {
	...
	static function checkRayDirectionAgainstReferenceVector(rayDirection:Vector3, refVector:Vector3, assert:Assert):Void {
	...
	static function main() {
		// INSTANCING
		Assert.test("Instancing", function(assert:Assert) {
			// no params
		...
		// PROPERTIES
		Assert.todo("ray", function(assert:Assert) {
			assert.ok(false, 'everything\'s gonna be alright');
		});

		Assert.todo("near", function(assert:Assert) {
			assert.ok(false, 'everything\'s gonna be alright');
		});

		Assert.todo("far", function(assert:Assert) {
			assert.ok(false, 'everything\'s gonna be alright');
		});

		Assert.todo("camera", function(assert:Assert) {
			assert.ok(false, 'everything\'s gonna be alright');
		});

		Assert.todo("layers", function(assert:Assert) {
			assert.ok(false, 'everything\'s gonna be alright');
		});

		Assert.todo("params", function(assert:Assert) {
			assert.ok(false, 'everything\'s gonna be alright');
		});

		// PUBLIC
		Assert.test("set", function(assert:Assert) {
		...
		Assert.test("setFromCamera (Perspective)", function(assert:Assert) {
			...
			var step = 0.1;
			for (x in -1 to 1 by step) {
				for (y in -1 to 1 by step) {
					raycaster.setFromCamera({
						x: x,
						y: y
					}, camera);
		...
		Assert.test("setFromCamera (Orthographic)", function(assert:Assert) {
			var raycaster = new Raycaster();
		...
		Assert.test("intersectObject", function(assert:Assert) {
		...
		Assert.test("intersectObjects", function(assert:Assert) {
			var raycaster = getRaycaster();
			var objectsToCheck = getObjectsToCheck();
		...
		Assert.test("Line intersection threshold", function(assert:Assert) {
			var raycaster = getRaycaster();
		...
		Assert.test("Points intersection threshold", function(assert:Assert) {
			var raycaster = getRaycaster();
