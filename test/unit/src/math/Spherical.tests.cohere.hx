...
import js.QUnit;
import js.Math.Spherical;
import js.Math.Vector3;

class SphericalTest {
    static function main() {
        QUnit.module('Maths', function() {
            QUnit.module('Spherical', function() {
                // INSTANCING
                ...
                QUnit.test('setFromVector3', function(assert) {
                    ...
                    a.setFromVector3(c);
                    assert.isTrue(Math.abs(a.radius - expected.radius) <= eps, 'Normal vector: check radius');
                    assert.isTrue(Math.abs(a.phi - expected.phi) <= eps, 'Normal vector: check phi');
                    assert.isTrue(Math.abs(a.theta - expected.theta) <= eps, 'Normal vector: check theta');
                });

                QUnit.test('setFromCartesianCoords', function(assert) {
                    var a = new Spherical(1, 1, 1);
                    var expected = new Spherical(4.554032147688322, 1.3494066171539107, 2.356194490192345);
