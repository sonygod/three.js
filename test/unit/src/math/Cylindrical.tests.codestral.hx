        ...
        QUnit.module("Maths", () -> {
            QUnit.module("Cylindrical", () -> {
                ...
                QUnit.test("setFromVector3", (assert) -> {
                    ...
                    a.setFromVector3(c);
                    assert.ok(Math.abs(a.radius - expected.radius) <= MathConstants.EPS, 'Normal vector: check radius');
                    assert.ok(Math.abs(a.theta - expected.theta) <= MathConstants.EPS, 'Normal vector: check theta');
                    assert.ok(Math.abs(a.y - expected.y) <= MathConstants.EPS, 'Normal vector: check y');
                });
