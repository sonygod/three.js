...
import three.js.test.unit.src.math.Cylindrical;
import three.js.test.unit.src.math.Vector3;
import three.js.test.unit.src.utils.math-constants;
        ...
        QUnit.module('Maths', () -> {
            ...
            QUnit.module('Cylindrical', () -> {
                ...
                QUnit.test('setFromVector3', (assert) -> {
                    ...
                    a.setFromVector3(c);
                    assert.ok(Math.abs(a.radius - expected.radius) <= math-constants.eps, 'Normal vector: check radius');
                    assert.ok(Math.abs(a.theta - expected.theta) <= math-constants.eps, 'Normal vector: check theta');
                    assert.ok(Math.abs(a.y - expected.y) <= math-constants.eps, 'Normal vector: check y');
