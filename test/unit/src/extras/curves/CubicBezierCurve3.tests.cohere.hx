...
import js.three.extras.curves.CubicBezierCurve3;
...
class Test {
    static function main() {
        ...
        $assert.isTrue(object instanceof Curve, 'CubicBezierCurve3 extends from Curve');

        // INSTANCING
        $assert.isTrue(curve != null, 'Can instantiate a CubicBezierCurve3.');

        // PROPERTIES
        $assert.isTrue(curve.type == 'CubicBezierCurve3', 'CubicBezierCurve3.type should be CubicBezierCurve3');

        // PUBLIC
        $assert.isTrue(curve.isCubicBezierCurve3, 'CubicBezierCurve3.isCubicBezierCurve3 should be true');
        ...
        $assert.isTrue(points.length == expectedPoints.length, 'Correct number of points');
        $assert.isTrue(points == expectedPoints, 'Correct points calculated');

        // symmetry
        var curveRev = new CubicBezierCurve3(curve.v3, curve.v2, curve.v1, curve.v0);

        points = curveRev.getPoints(expectedPoints.length - 1);

        $assert.isTrue(points.length == expectedPoints.length, 'Reversed: Correct number of points');
        $assert.isTrue(points == expectedPoints.reverse(), 'Reversed: Correct points curve');
        ...
        $assert.isTrue(lengths.length == expectedLengths.length, 'Correct number of segments');
        ...
        $assert.isTrue(points == expectedPoints, 'Correct points');
        ...
        for (i in 0...expectedTangents.length) {
            ...
            $assertCoefficient(tangent.y, exp.y, 'getTangentAt #' + i + ': y correct');
        }
        ...
        $assert.isTrue(start == 0, 'getUtoTmapping( 0, 0 ) is the starting point');
        $assert.isTrue(end == 1, 'getUtoTmapping( 0, length ) is the ending point');
        ...
        for (group in expected.keys()) {
            var groupVecs = expected[group];

            for (i in 0...groupVecs.length) {
                var exp = groupVecs[i];
                var vec = frames[group][i];

                $assert.numEqual(vec.x, exp.x, 'Frenet frames [' + i + ', ' + j + '].x correct');
                $assert.numEqual(vec.y, exp.y, 'Frenet frames [' + i + ', ' + j + '].y correct');
                $assert.numEqual(vec.z, exp.z, 'Frenet frames [' + i + ', ' + j + '].z correct');
            }
        }
    }
}
...
function $assertCoefficient(a:Float, b:Float, msg:String) {
    if (Math.abs(a - b) / a > 0.001) throw 'Assertion failed: ' + msg;
}

function $assertNumEqual(a:Float, b:Float, msg:String) {
    if (Math.abs(a - b) > 0.001) throw 'Assertion failed: ' + msg;
}

function $assertNumNotEqual(a:Float, b:Float, msg:String) {
    if (Math.abs(a - b) < 0.001) throw 'Assertion failed: ' + msg;
}
