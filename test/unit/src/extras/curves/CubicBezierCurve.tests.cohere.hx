...
import js.extras.curves.CubicBezierCurve;
	...
	public static function main() {
		QUnit.module('Extras', function() {
			QUnit.module('Curves', function() {
				QUnit.module('CubicBezierCurve', function(hooks) {
					...
					QUnit.todo('v3', function(assert) {
						// Vector2 exists
						assert.ok(false, 'everything\'s gonna be alright');
					});

					// PUBLIC
					QUnit.test('isCubicBezierCurve', function(assert) {
						var object = new CubicBezierCurve();
						assert.ok(
					...
					QUnit.test('getLength/getLengths', function(assert) {
						...
						for (i in 0...lengths.length) {
							...
							assert.numEqual(segment, exp, 'segment[' + i + '] correct');
						}
					});
					...
					QUnit.test('getTangent/getTangentAt', function(assert) {
						...
						for (i in 0...expectedTangents.length) {
							...
							assertFreq.numEqual(tangent.y, exp.y, 'getTangentAt #' + i + ': y correct');
						}
					});
