package three.js.test.unit.src.extras.curves;

import haxe.unit.TestCase;
import three.js.extras.curves.CubicBezierCurve3;
import three.js.extras.core.Curve;
import three.js.math.Vector3;

class CubicBezierCurve3Tests {
    public function new() {}

    public function testExtending():Void {
        var curve = new CubicBezierCurve3(
            new Vector3(-10, 0, 2),
            new Vector3(-5, 15, 4),
            new Vector3(20, 15, -5),
            new Vector3(10, 0, 10)
        );
        assertTrue(curve instanceof Curve, 'CubicBezierCurve3 extends from Curve');
    }

    public function testInstancing():Void {
        var curve = new CubicBezierCurve3();
        assertNotNull(curve, 'Can instantiate a CubicBezierCurve3.');
    }

    public function testType():Void {
        var curve = new CubicBezierCurve3();
        assertEquals(curve.type, 'CubicBezierCurve3', 'CubicBezierCurve3.type should be CubicBezierCurve3');
    }

    // Todo: implement v0, v1, v2, v3 tests
    public function testV0():Void {
        fail("Todo: implement v0 test");
    }

    public function testV1():Void {
        fail("Todo: implement v1 test");
    }

    public function testV2():Void {
        fail("Todo: implement v2 test");
    }

    public function testV3():Void {
        fail("Todo: implement v3 test");
    }

    public function testIsCubicBezierCurve3():Void {
        var curve = new CubicBezierCurve3();
        assertTrue(curve.isCubicBezierCurve3, 'CubicBezierCurve3.isCubicBezierCurve3 should be true');
    }

    // Todo: implement getPoint test
    public function testGetPoint():Void {
        fail("Todo: implement getPoint test");
    }

    // Todo: implement copy test
    public function testCopy():Void {
        fail("Todo: implement copy test");
    }

    // Todo: implement toJSON test
    public function testToJSON():Void {
        fail("Todo: implement toJSON test");
    }

    // Todo: implement fromJSON test
    public function testFromJSON():Void {
        fail("Todo: implement fromJSON test");
    }

    public function testSimpleCurve():Void {
        var curve = new CubicBezierCurve3(
            new Vector3(-10, 0, 2),
            new Vector3(-5, 15, 4),
            new Vector3(20, 15, -5),
            new Vector3(10, 0, 10)
        );
        var expectedPoints:Array<Vector3> = [
            new Vector3(-10, 0, 2),
            new Vector3(-3.359375, 8.4375, 1.984375),
            new Vector3(5.625, 11.25, 1.125),
            new Vector3(11.796875, 8.4375, 2.703125),
            new Vector3(10, 0, 10)
        ];
        var points:Array<Vector3> = curve.getPoints(expectedPoints.length - 1);
        assertEquals(points.length, expectedPoints.length, 'Correct number of points');
        assertEquals(points, expectedPoints, 'Correct points calculated');

        // symmetry
        var curveRev = new CubicBezierCurve3(
            curve.v3, curve.v2, curve.v1, curve.v0
        );
        points = curveRev.getPoints(expectedPoints.length - 1);
        assertEquals(points.length, expectedPoints.length, 'Reversed: Correct number of points');
        assertEquals(points, expectedPoints.reverse(), 'Reversed: Correct points curve');
    }

    public function testGetLengthGetLengths():Void {
        var curve = new CubicBezierCurve3(
            new Vector3(-10, 0, 2),
            new Vector3(-5, 15, 4),
            new Vector3(20, 15, -5),
            new Vector3(10, 0, 10)
        );
        var length:Float = curve.getLength();
        var expectedLength:Float = 39.58103024989427;
        assertEquals(length, expectedLength, 'Correct length of curve');

        var expectedLengths:Array<Float> = [
            0,
            10.73729718231036,
            20.19074500737662,
            27.154413277853756,
            38.453287150114214
        ];
        var lengths:Array<Float> = curve.getLengths(expectedLengths.length - 1);
        assertEquals(lengths.length, expectedLengths.length, 'Correct number of segments');
        for (i in 0...lengths.length) {
            assertEquals(lengths[i], expectedLengths[i], 'segment[' + i + '] correct');
        }
    }

    public function testGetPointAt():Void {
        var curve = new CubicBezierCurve3(
            new Vector3(-10, 0, 2),
            new Vector3(-5, 15, 4),
            new Vector3(20, 15, -5),
            new Vector3(10, 0, 10)
        );
        var expectedPoints:Array<Vector3> = [
            new Vector3(-10, 0, 2),
            new Vector3(-2.591880240484318, 8.908333501170798, 1.8953420625251136),
            new Vector3(4.866251460832755, 11.22787914038507, 1.150832855206874),
            new Vector3(10, 0, 10)
        ];
        var points:Array<Vector3> = [
            curve.getPointAt(0, new Vector3()),
            curve.getPointAt(0.3, new Vector3()),
            curve.getPointAt(0.5, new Vector3()),
            curve.getPointAt(1, new Vector3())
        ];
        assertEquals(points, expectedPoints, 'Correct points');
    }

    public function testGetTangentGetTangentAt():Void {
        var curve = new CubicBezierCurve3(
            new Vector3(-10, 0, 2),
            new Vector3(-5, 15, 4),
            new Vector3(20, 15, -5),
            new Vector3(10, 0, 10)
        );
        var expectedTangents:Array<Vector3> = [
            new Vector3(0.3138715439944244, 0.9411440474105875, 0.12542940601858074),
            new Vector3(0.8351825262580098, 0.54174002562179, -0.09480449605683638),
            new Vector3(0.9997531780538501, 0, -0.02221672728433752),
            new Vector3(0.40693407933981185, -0.7512629496079668, 0.5196235518317053),
            new Vector3(-0.42632467075185815, -0.6396469221230213, 0.6396085444448543)
        ];
        var tangents:Array<Vector3> = [
            curve.getTangent(0, new Vector3()),
            curve.getTangent(0.25, new Vector3()),
            curve.getTangent(0.5, new Vector3()),
            curve.getTangent(0.75, new Vector3()),
            curve.getTangent(1, new Vector3())
        ];
        for (i in 0...tangents.length) {
            assertEquals(tangents[i].x, expectedTangents[i].x, 'getTangent #' + i + ': x correct');
            assertEquals(tangents[i].y, expectedTangents[i].y, 'getTangent #' + i + ': y correct');
        }

        // ...
    }

    public function testGetUtoTmapping():Void {
        var curve = new CubicBezierCurve3(
            new Vector3(-10, 0, 2),
            new Vector3(-5, 15, 4),
            new Vector3(20, 15, -5),
            new Vector3(10, 0, 10)
        );
        var start:Float = curve.getUtoTmapping(0, 0);
        var end:Float = curve.getUtoTmapping(0, curve.getLength());
        var somewhere:Float = curve.getUtoTmapping(0.5, 1);
        var expectedSomewhere:Float = 0.021163245321323316;
        assertEquals(start, 0, 'getUtoTmapping( 0, 0 ) is the starting point');
        assertEquals(end, 1, 'getUtoTmapping( 0, length ) is the ending point');
        assertEquals(somewhere, expectedSomewhere, 'getUtoTmapping( 0.5, 1 ) is correct');
    }

    public function testGetSpacedPoints():Void {
        var curve = new CubicBezierCurve3(
            new Vector3(-10, 0, 2),
            new Vector3(-5, 15, 4),
            new Vector3(20, 15, -5),
            new Vector3(10, 0, 10)
        );
        var expectedPoints:Array<Vector3> = [
            new Vector3(-10, 0, 2),
            new Vector3(-5.756524515061918, 6.568020242700483, 2.22116711170301),
            new Vector3(1.0003511895116906, 10.49656064587831, 1.4727101010850698),
            new Vector3(8.767656412295171, 10.784286845278622, 1.2873599519775174),
            new Vector3(12.306772513558396, 5.545103788071547, 4.909948454535794),
            new Vector3(10, 0, 10)
        ];
        var points:Array<Vector3> = curve.getSpacedPoints();
        assertEquals(points.length, expectedPoints.length, 'Correct number of points');
        assertEquals(points, expectedPoints, 'Correct points calculated');
    }

    public function testComputeFrenetFrames():Void {
        var curve = new CubicBezierCurve3(
            new Vector3(-10, 0, 2),
            new Vector3(-5, 15, 4),
            new Vector3(20, 15, -5),
            new Vector3(10, 0, 10)
        );
        var expected = {
            binormals: [
                new Vector3(-0.9486358543207215, 0.316370061632252, -6.938893903907228e-18),
                new Vector3(-0.05491430765311864, 0.9969838307670049, 0.054842137122173326),
                new Vector3(0.5944656510461876, 0.334836503700931, 0.7310917216844742)
            ],
            normals: [
                new Vector3(0.03968210891259515, 0.11898683173537697, -0.9921025471723304),
                new Vector3(-0.047981365124836806, 0.05222670079466692, -0.9974819097732357),
                new Vector3(0.6818048583242511, -0.6919077473246573, -0.23749906180354932)
            ],
            tangents: [
                new Vector3(0.3138715439944244, 0.9411440474105875, 0.12542940601858074),
                new Vector3(0.9973375594129282, 0.05740742907719315, -0.04496865209244443),
                new Vector3(-0.42632467075185815, -0.6396469221230213, 0.6396085444448543)
            ]
        };
        var frames = curve.computeFrenetFrames(2, false);
        for (group in ['binormals', 'normals', 'tangents']) {
            for (i in 0...expected[group].length) {
                assertEquals(frames[group][i].x, expected[group][i].x, 'Frenet frames [' + i + '].x correct');
                assertEquals(frames[group][i].y, expected[group][i].y, 'Frenet frames [' + i + '].y correct');
                assertEquals(frames[group][i].z, expected[group][i].z, 'Frenet frames [' + i + '].z correct');
            }
        }
    }
}